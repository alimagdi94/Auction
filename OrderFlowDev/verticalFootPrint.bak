...
input int    InpBarGapPx        = 2;            // Gap between footprint columns
input color  InpSeparatorColor  = C'220,220,220';  // Bar separator line color
input color  InpBarOutlineColor = C'220,220,220';  // Bar column outline color

input group "Colors - Heatmap"
input color  InpBidBaseColor   = C'200,50,50';     // Bid Volume Base
input color  InpBidHighColor   = clrRed;           // Bid Volume High
input color  InpAskBaseColor   = C'50,200,50';     // Ask Volume Base
input color  InpAskHighColor   = clrForestGreen;   // Ask Volume High
input color  InpOutOfVAColor   = C'40,40,40';      // Out of Value Area Color (grid cell bg)
input color  InpEmptyCellColor = C'40,40,40';      // Grid cell background with 0 volume
...
void DrawGridCell(int x_left, int y_top, int barWidthPx, int fullTrackW, string sBid, string sAsk, color bgBid, color bgAsk, string bidFontStr, string askFontStr, color txtCol, int histWidthPx, color histColor)
  {
   ...
   ObjectSetInteger(g_chart, g_pool[idx].bg_track, OBJPROP_BGCOLOR, C'35,35,45');
   ObjectSetInteger(g_chart, g_pool[idx].bg_track, OBJPROP_COLOR, C'35,35,45');
   ...
   ObjectSetInteger(g_chart, g_pool[idx].vgrid_line, OBJPROP_BGCOLOR, clrBlack);
   ObjectSetInteger(g_chart, g_pool[idx].vgrid_line, OBJPROP_COLOR, clrBlack);
   ...
   ObjectSetInteger(g_chart, g_pool[idx].hgrid_line, OBJPROP_BGCOLOR, clrBlack);
   ObjectSetInteger(g_chart, g_pool[idx].hgrid_line, OBJPROP_COLOR, clrBlack);
      ...
      for(int i = 0; i < stepsNum; i++)
        {
         ...
         string bidFontStr = "Consolas Bold";
         string askFontStr = "Consolas Bold";
         string sBid = " ";
         string sAsk = " ";
         ...
         if(found && (bV > 0 || aV > 0)) {
            ...
            if (pl.is_imb_sell) { bidFontStr = "Consolas Bold"; txtCol = InpImbSellColor; }
            if (pl.is_imb_buy) { askFontStr = "Consolas Bold"; txtCol = InpImbBuyColor; }
