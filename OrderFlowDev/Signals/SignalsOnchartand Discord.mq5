...
//| Visual-only pass: renders a frozen signal card for every entry  |
//| in g_firedSignals[] that is currently visible on screen.        |
//|                                                                  |
//| IMPORTANT: this function NEVER calls ComputeHFTSignal or        |
//| ComputeOFScore. All values are read from the immutable cache    |
//| that was written when the signal originally fired — so cards    |
//| are pixel-stable across every render frame.                     |
//+------------------------------------------------------------------+
void DrawSignalMarkersPass(int visBars, int firstVis, int barW)
  {
   ...
   // Build a set of visible bar_times for fast O(vis) lookup
   // (avoids scanning g_firedSignals for every bar)
   for(int fi = 0; fi < g_firedCount; fi++)
     {
      SignalRecord sig = g_firedSignals[fi];  // local copy — MQL5 forbids &ref to array element

      // Find bar shift for this signal
      int shift = iBarShift(_Symbol, PERIOD_CURRENT, sig.bar_time);
      if(shift < 0)
         continue;
      // Skip if outside the currently visible window
      if(shift > firstVis || shift < firstVis - visBars + 1)
         continue;
      // Confirm bar still exists in our array (sanity check)
      int bi = FindBarIndex(sig.bar_time);
      if(bi < 0)
         continue;

      //--- Screen geometry from frozen bar_high / bar_low ---
      ...
      // Bar centre X from any level price (levels are sorted descending)
      ...
      color bgDark   = isBuy ? C'3,22,12'        : C'22,3,8';
      color bgHeader = isBuy ? C'5,38,20'        : C'38,5,14';
      ...
      uint  cScoreV  = FpARGB(isBuy ? C'80,255,150' : C'255,100,110', 255);

      //--- Build card text from frozen snapshot — identical to Discord ---
      string arrowStr = isBuy ? ShortToString(0x25B2) : ShortToString(0x25BC);
      ...
      string rowFooter = _Symbol + " \x00B7 " + tfStr + " \x00B7 " + timeStr;

      //--- Card layout ---
      int cardW  = MathMax(MathMin(barW * 2 + 40, 260), 192);
      int padX   = 8;
      int padY   = 5;
      int hdrH   = 18;
      int rowH   = 14;
      int cardH  = padY + hdrH + 1 + 3 + rowH + rowH + rowH + padY;
