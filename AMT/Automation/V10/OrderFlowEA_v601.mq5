//+------------------------------------------------------------------+
//|  OrderFlowEA_v601.mq5                                            |
//|  Order Flow EA — Pure Trading Engine v6.01                        |
//|  No canvas, no UI objects — Journal + chart markers only          |
//|                                                                    |
//|  Production changes from v6.00 (core logic unchanged):            |
...
//
// [FIX-9] The slPointsUnused parameter is accepted for API compatibility
//         (callers already compute slPoints) but is intentionally NOT used
//         for lot sizing — see design note above.  The parameter is renamed
//         from the silent /*slPoints*/ comment in v6.00 to make the intent
//         explicit and avoid confusion during code review.
//
...
int OnInit()
  {
   ...
   if(InpATEnable)
     {
      // CalcLot always uses InpSLPips as the risk reference regardless of SL mode.
      // Validate here so the lot formula never divides by zero.
      if(InpUseRiskPercent && InpSLPips <= 0.0)
        { Alert("OrderFlowEA: InpSLPips must be > 0 when dynamic risk sizing is enabled."); return INIT_PARAMETERS_INCORRECT; }
