...
//==========================================================================
// SECTION 6: TICK PROCESSING PIPELINE
//==========================================================================
...
//==========================================================================
// SECTION 7: LEVEL SORTING & PROFILE HELPERS
//==========================================================================
...
void ReloadHistory()
  {
   ...
   if(loaded > 0)
     {
      ComputeNakedPOCs();
      Print("OrderFlowEA — History loaded: ", loaded, " ticks across ",
            ArraySize(g_bars), " bars.");
     }
   else
     {
   ...
   if(httpCode == -1)
     {
      ...
      if(err == 4014)
         ...
         Print("OrderFlowEA — Discord WebRequest failed, error: ", err);
     }
   else if(httpCode != 204 && httpCode != 200)
      Print("OrderFlowEA — Discord returned HTTP ", httpCode);
  }
...
void EvalAndFireSignal()
  {
   ...
   Print("OrderFlowEA — SIGNAL: ", dir,
         " | Symbol: ", _Symbol, " | TF: ", tf,
         " | HFT: ", displayScore, " | OFS: ", currentScore,
         " | Price: ", DoubleToString(SymbolInfoDouble(_Symbol, SYMBOL_BID), _Digits),
         " | Bar: ", TimeToString(g_bars[bi].bar_time, TIME_DATE | TIME_MINUTES),
         " | Naked POC: ", (g_bars[bi].is_naked_poc ? "YES" : "NO"),
         " | DivDiv: ",    (g_bars[bi].is_delta_divergence ? "YES" : "NO"));
...
//==========================================================================
// SECTION 11: AUTOMATED TRADING ENGINE
//==========================================================================
...
void RefreshSymbolInfo()
  {
   ...
   if(g_TickSize <= 0.0)
     {
      Print("OrderFlowEA — Warning: tick value is zero for ", _Symbol,
   ...
   if(!TerminalInfoInteger(TERMINAL_TRADE_ALLOWED))
     {
      ...
      if(TimeCurrent() - s_t1 > 60)
        { Print("OrderFlowEA — Terminal trade not allowed (AutoTrading off?)."); s_t1 = TimeCurrent(); }
      return false;
     }
   if(!MQLInfoInteger(MQL_TRADE_ALLOWED))
     {
      ...
      if(TimeCurrent() - s_t2 > 60)
        { Print("OrderFlowEA — EA trade permission denied."); s_t2 = TimeCurrent(); }
      return false;
     }
   if(!AccountInfoInteger(ACCOUNT_TRADE_ALLOWED))
     {
      ...
      if(TimeCurrent() - s_t3 > 60)
        { Print("OrderFlowEA — Account trade not allowed (read-only or suspended?)."); s_t3 = TimeCurrent(); }
      return false;
     }
   return true;
  }
      ...
      switch(InpSLMode)
        {
         ...
         case SL_MODE_ATR:
            if(atrVal > 0.0)
              ...
              {
               Print("OrderFlowEA — CalcSLTP: ATR=0, falling back to Fixed-Pips SL (",
                     InpSLPips, " pips)");
               sl = isBuy ? NormalizeDouble(entry - InpSLPips * g_Pip, _Digits)
                          : NormalizeDouble(entry + InpSLPips * g_Pip, _Digits);
              }
   ...
   switch(InpTPMode)
     {
      ...
      case TP_MODE_ATR:
         if(atrVal > 0.0)
           ...
           {
            Print("OrderFlowEA — CalcSLTP: ATR=0, falling back to Fixed-Pips TP (",
                  InpTPPips, " pips)");
            tp = isBuy ? NormalizeDouble(entry + InpTPPips * g_Pip, _Digits)
                       : NormalizeDouble(entry - InpTPPips * g_Pip, _Digits);
           }
...
bool trade_OrderDelete(ulong ticket)
  {
   ...
   if(!ok)
      Print("OrderFlowEA — OrderDelete failed: ticket=", ticket,
            " retcode=", res.retcode);
   return ok;
  }
...
bool trade_Send(ENUM_TRADE_REQUEST_ACTIONS action,
                ...
                string                     comment)
  {
   ...
   if(!SymbolInfoTick(_Symbol, lastTick))
     {
      Print("OrderFlowEA — trade_Send: SymbolInfoTick failed (", GetLastError(), ")");
      return false;
     }
   ...
   if(action == TRADE_ACTION_PENDING)
     {
      if(orderType == ORDER_TYPE_BUY_STOP && price <= freshAsk)
        {
         Print("OrderFlowEA — BuyStop skipped: entry (", price,
               ") is not above current ask (", freshAsk, ")");
         return false;
        }
      if(orderType == ORDER_TYPE_SELL_STOP && price >= freshBid)
        {
         Print("OrderFlowEA — SellStop skipped: entry (", price,
   ...
   if(!ok)
      Print("OrderFlowEA — OrderSend failed: retcode=", res.retcode,
...
void PlaceOrders()
  {
   ...
   if(InpMaxEquityProfit > 0.0 && equity >= balance + InpMaxEquityProfit)
     {
      Print("OrderFlowEA — MaxEquityProfit reached. Auto-trading halted.");
      g_autoTrade = false;
      return;
     }
   if(InpMaxEquityLoss > 0.0 && equity <= balance - InpMaxEquityLoss)
     {
      Print("OrderFlowEA — MaxEquityLoss hit. Auto-trading halted.");
      g_autoTrade = false;
      return;
     }
   ...
   if(!SymbolInfoTick(_Symbol, lv))
     { Print("OrderFlowEA — PlaceOrders: SymbolInfoTick failed"); return; }
...
//--- Real-time break-even and trailing stop management (every tick)
      ...
      if(MathAbs(newSL - curSL) > _Point / 2.0)
        {
         ...
         if(!modOk)
            Print("OrderFlowEA — ManagePositions modify failed: ticket=", ticket,
                  " retcode=", res.retcode,
                  " newSL=", req.sl, " tp=", req.tp);
        }
     }
  }

//==========================================================================
// SECTION 12: EA LIFECYCLE HANDLERS
//==========================================================================
...
int OnInit()
  {
   ...
   if(g_handleATR == INVALID_HANDLE)
      Print("OrderFlowEA — Warning: ATR indicator handle could not be created (",
            GetLastError(), ").");

   ReloadHistory();

   Print("OrderFlowEA — Initialised | Symbol: ", _Symbol,
         " | TickSize: ", g_basePts, "pts × ", g_tickMult,
         " | Step: ", DoubleToString(g_step / _Point, 0), "pts",
         " | HistoryBars: ", g_histBars,
         " | AutoTrade: ", (g_autoTrade ? "ON" : "OFF"),
         " | HasTrades: ", (g_hasTrades ? "YES" : "NO"));
...
void OnDeinit(const int reason)
  {
   ...
   Print("OrderFlowEA — Deinitialised (reason=", reason, ").");
  }
