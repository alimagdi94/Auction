//+------------------------------------------------------------------+
//|  OrderFlowEA_v803.mq5                                            |
//|  Order Flow EA — Production Grade v8.03                          |
//|                                                                    |
//|  v8.03 — March 2026 code-review patch (4 fixes applied).        |
//|  All v8.02 logic preserved; this release addresses:              |
//|                                                                    |
//|  [V8-18] SESSION HALT NEVER RESET — CRITICAL (BUG A)            |
//|    g_sessionHalted was never set back to false after triggering.  |
//|    Combined with V8-10 GlobalVariable persistence, this made      |
...
#property description "Order Flow EA v8.03 — Production-Grade (Code Review Fixes Applied)"
#property strict

// [V8-18] Single source of truth for the EA version string. Reference this in all
//    Alert() and log calls so version labels stay accurate across future patches.
...
void ComputeBarSignals(int bi)
  {
   ...
   // [V8-18] BUG FIX (BUG D): The original code scanned ask_vol starting from index 0
   //    (bar LOW) and bid_vol from index len-1 (bar HIGH). Near-zero ask_vol at the LOW
   //    is normal market structure on any bar that rallied — buyers don't press the offer
   //    at the bar's extreme low because price has already moved away. This caused false
   //    positives on most upward-moving bars.
   //    Standard footprint interpretation:
   //      - Near-zero ASK volume at the bar HIGH = buyers exhausted at the top (bearish).
   //      - Near-zero BID volume at the bar LOW  = sellers exhausted at the bottom (bullish).
   //    Fix: swap the scan directions. askRun now starts from the TOP (len-1 downward)
   //    and bidRun starts from the BOTTOM (0 upward).
...
int ComputeOFScore(int bi)
  {
   ...
   // [V8-17] BUG FIX: Determine absorption direction from location, not bar close direction.
   //    A bar that closes up but has absorption only at the HIGH is a bearish structure
   //    (sellers absorbed buying). Using bar direction as proxy mis-scores such bars bullish.
   //    Mirror the C4 logic from ComputeHFTSignal(): absorption at LOW = bullish (buyers
   //    absorbed selling pressure); absorption at HIGH = bearish (sellers absorbed buying).
   bool absOfsLow = false, absOfsHigh = false;
   {
      int chkA = MathMin(3, len/3+1);
      for(int i = len-chkA; i < len; i++) if(g_bars[bi].levels[i].is_absorption) absOfsLow  = true;
      for(int i = 0;        i < chkA; i++) if(g_bars[bi].levels[i].is_absorption) absOfsHigh = true;
   }
...
double ComputeHFTSignal(int bi)
  {
   ...
   // C3: POC gravity (15%)
   // [V8-17] BUG FIX: POC at LOW = bullish (accumulation); POC at HIGH = bearish (distribution).
   //    levels[] is sorted price-ascending (index 0 = lowest price level), so poc_idx near
   //    len-1 means POC at the HIGH — a distribution zone that is bearish, not bullish.
   //    The previous formula assigned +1.0 for POC-at-HIGH, which was the wrong sign.
   double c3 = 0.0;
...
// [V8-07] The snapshot is deferred until all positions for this magic number are
//    closed, so an overnight position that closes at the day open does not
//    produce a stale balance snapshot.
bool CheckDailyLoss()
  {
   ...
   if(dt.day != g_dayStartDay)
     {
      // [V8-07] Only snapshot once the previous session's positions have settled.
      ...
      if(!positionsOpen)
        {
         ...
         // [V8-18] BUG FIX (BUG A): g_sessionHalted was never reset, making
         //    InpHaltConsecLosses a permanent kill switch rather than a session-level
         //    protection. The GlobalVariable persistence (V8-10) also restored the
         //    halted state across restarts and days with no recovery path. Fix: reset
         //    both the halt flag and the counter that triggered it on each new day,
         //    matching the documented intent of "session halt resets on the next day".
         g_sessionHalted     = false;
         g_consecutiveLosses = 0;
         RiskStateSave();   // [V8-10]
         LogRisk(StringFormat("New day — DayStartBalance: %.2f | DailyLossLimit: %.1f%% | SessionHalt + ConsecLoss counter reset. [V8-07/V8-18]",
