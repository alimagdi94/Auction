...
All nine v8.07 passes are correctly implemented. `ComputeCVDSlope()` is a clean extraction that genuinely eliminates the synchronization risk. `UA-SCORE` resolves the conviction/score mismatch exactly as specified. `SL-COOLDOWN-PERSIST` and `SL-COMMENT-CASE` close two real reliability gaps. The delta threshold exposure, `ARRAYRESIZE` pre-allocation, `NEWDAY-DEFER` deadline, and `CTR-ANALYSIS` fix are all mechanically sound.

Four new issues are identified, none critical. The most significant are: `InpDeltaConvThreshold` is exposed as an input but never validated in `OnInit()`, meaning an out-of-range value silently disables a conviction component entirely; `CounterSave()` is gated under `InpShowVisuals`, which means `g_virtualTicket` accumulates in analysis mode without being persisted when visuals are off; `g_newDayDeferStart` is not saved to GlobalVariables, so an EA restart during a deferred new-day resets the 24-hour clock; and the header changelog contains an orphaned text block from v8.21's DEAD-UA entry that lost its `[VX-XX]` tag prefix during the v8.07 edit pass.

Three low-severity issues from v8.06 are carried forward unchanged: C3 POC gravity uses level index rather than price distance, `AccumulateTick()` still runs a linear level search on the tick hot path, and the signal-vs-order bar evaluation asymmetry remains undocumented.

Overall quality advances from 8.0 to **8.5/10**.

---

## Section 1 — Pass-by-Pass Verification

### Pass 1 — Version Bump
**Status: Correct.**  
`#property version "8.07"`, `#define EA_VERSION "8.07"`, and `EA_NAME` all updated consistently. V8-22 changelog block prepended in full.

**One defect in the changelog itself** (see Section 2.4): the old DEAD-UA body text from v8.21 appears at lines 65–71 without a `[VX-XX]` tag prefix — a copy-paste artifact left over from the pass that inserted the `SL-COMMENT-CASE` block above it.

---

### Pass 2 — SL-COOLDOWN-PERSIST
**Status: Correct.**

`RiskStateSave()` (lines 624–625):
```mql5
GlobalVariableSet(GVKey("LastSLBuy"),  (double)g_lastSLBarTimeBuy);
GlobalVariableSet(GVKey("LastSLSell"), (double)g_lastSLBarTimeSell);
```
`RiskStateLoad()` (lines 649–652):
```mql5
...
The `double` ↔ `datetime` cast round-trip is safe: `datetime` is a 32-bit integer on most MT5 builds (unix epoch), and IEEE-754 double stores integers exactly up to 2^53. Unix timestamps won't exceed that boundary for centuries.

`CheckSLCooldown()` reads the restored timestamp and calls `iBarShift(_Symbol, PERIOD_CURRENT, lastSLBarTime)`. After a restart, this correctly computes the number of bars since the SL hit from the stored bar-open time. The cooldown is now fully persistent. ✅

**One residual point** (see Section 2.3): `g_newDayDeferStart` is a new datetime global added in this release and follows the same pattern as these timestamps — yet it is not included in `RiskStateSave()`/`RiskStateLoad()`.
...
`ComputeCVDSlope()` (lines 1639–1652):
```mql5
...
Both call sites updated correctly:
- `ComputeHFTSignal()` C6 (line 1720): `double slope = ComputeCVDSlope(bi);` followed by `slope * 2.25` amplifier and clamp.
- `GetConvictionResult()` Component 4 (line 1821): `double slope = ComputeCVDSlope(bi);` followed by `MathIsValidNumber(slope)` guard and threshold comparison.

The helper's own `bi < 2` guard makes the callers' outer `if(bi >= 2)` checks redundant but not wrong — belt-and-suspenders is acceptable for a function this critical.

`MathIsValidNumber(slope)` is checked in `GetConvictionResult()` but not in `ComputeHFTSignal()`. The return value of `ComputeCVDSlope()` is either 0.0 (early return) or an arithmetic expression over guarded IEEE-754 doubles (nd0/nd1/nd2 are all clamped to 0.0 if invalid), so NaN or Inf cannot be produced. The missing check in `ComputeHFTSignal()` is safe but asymmetric. ✅

---

### Pass 4 — ARRAYRESIZE
**Status: Partially correct. Inconsistency noted** (see Section 2.5).

`ArrayResize(tags, 0, 12)` at line 1743 correctly pre-allocates 12 slots before any tag is appended. This is the right approach: MQL5 honours the reserved capacity on subsequent `ArrayResize` calls that grow within that bound, avoiding per-call heap reallocation.

However, 10 of the 14 subsequent `ArrayResize(tags, n+1)` calls do not include the reserve parameter (lines 1758, 1793, 1795, 1797, 1801, 1806, 1808, 1810, 1814, 1833). Four do include `ArrayResize(tags, n+1, 12)` (lines 1769, 1771, 1825, 1827). In practice, since the maximum tag count is bounded at ~10 and the initial reserve is 12, no reallocation will occur. The inconsistency is cosmetic but signals the fix was applied selectively during the pass rather than uniformly.
...
Component count loop (lines 1849–1852):
```mql5
...
UA_Lo and UA_Hi are now excluded from `componentCount` using the established NakedPOC carve-out pattern, while still being included in the label string for display. A borderline trade can no longer pass the `InpMinConvictionComp` gate solely on the basis of a signal with 0% HFT score weight. ✅

---

### Pass 6 — CTR-ANALYSIS
**Status: Correct for the stated case. Partial coverage gap** (see Section 2.1).

`CounterSave()` (lines 666–670):
```mql5
void CounterSave()
  {
...
Called at line 2122 immediately after `g_sigMarkerCount++` inside the `if(InpShowVisuals)` block in `EvalAndFireSignal()`. Since `g_sigMarkerCount` is only incremented when `InpShowVisuals` is true (the increment is inside `DrawSignalMarker(g_sigMarkerCount++, ...)`), gating `CounterSave()` on the same condition is logically consistent for `g_sigMarkerCount`.

However, `g_virtualTicket` is incremented in `PlaceOrders()` (line 2624: `g_virtualTicket++`) in analysis mode, independently of `InpShowVisuals`. `CounterSave()` is never called from `PlaceOrders()`. If analysis mode is active and `InpShowVisuals=false`, `g_virtualTicket` accumulates in memory and resets to 900M on every restart — the original crash-collision problem for virtual tickets is unresolved in this configuration. See Section 2.1.

---

### Pass 7 — NEWDAY-DEFER
**Status: Correct for the live-trading path. Persistence gap noted** (see Section 2.3).

`CheckNewDay()` (lines 1974–1980):
```mql5
if(positionsOpen)
  {
...
The 86400-second (24-hour) deadline is correctly enforced. The `g_newDayDeferStart = 0` reset after the snapshot fires prevents the deadline from being triggered again on the same day. The `LogRisk()` call on forced snapshot is good practice — users will see this in the journal on the next `LOG_TRADES_ONLY` restart after a gap event. ✅

---

### Pass 8 — DELTA-THRESHOLD
**Status: Correct for runtime use. Validation gap noted** (see Section 2.2).

Input declaration (line 397):
```mql5
input double InpDeltaConvThreshold = 0.35;
```

Usage in `GetConvictionResult()` Component 2 (lines 1768–1771):
```mql5
...
The substitution is clean and the parameter name is self-documenting. Default 0.35 preserves the prior hardcoded behavior exactly. The input is grouped correctly with other signal quality filters. `OnInit()` does not validate it — see Section 2.2.

---

### Pass 9 — SL-COMMENT-CASE
**Status: Correct.**

`OnTradeTransaction()` (lines 3067–3078):
```mql5
...
`StringToLower` applied once to a copy, then all three `StringFind` calls use `commentLower`. The original `comment` is preserved for the `LogTradeClosed()` call at line 3133 — important for debuggability so the journal shows the broker's actual comment string. ✅

One subtle point: `StringToLower` in MQL5 modifies the string in-place via a reference. The code correctly passes `commentLower` (a copy) not `comment` itself. The original is safe.

---

## Section 2 — New Issues

### 2.1 — `g_virtualTicket` Not Persisted When InpShowVisuals=false in Analysis Mode (Low)
...
### 2.2 — `InpDeltaConvThreshold` Not Validated in OnInit() (Low)
...
### 2.3 — `g_newDayDeferStart` Not Persisted Across Restarts (Low)
...
### 2.4 — Orphaned DEAD-UA Changelog Block in File Header (Very Low)
