//+------------------------------------------------------------------+
//|  OrderFlowEA_v806.mq5                                            |
//|  Order Flow EA — Production Grade v8.06                          |
//|                                                                    |
//|  v8.06 — March 2026 code-review patch (4 fixes applied).        |
//|  All v8.04 logic preserved; this release addresses:              |
//|                                                                    |
//|  [V8-21] DEAD-UA WIRED: Unfinished Auction flags consumed        |
...
#property description "Order Flow EA v8.06 — Production-Grade (Code Review Fixes Applied)"
#property strict

// [V8-18] Single source of truth for the EA version string.
#define EA_VERSION "8.06"
...
double ComputeHFTSignal(int bi, int preOFS = -1)
  {
   ...
   // [V8-20] Include bar bi-1 explicitly: prior formula (nd[bi]-nd[bi-2])/2 was the
   //    centered-difference endpoint formula and never accessed bi-1's delta. A sharp
   //    reversal at bi-1 that recovered at bi produced a near-zero slope, hiding the
   //    intra-window momentum swing. Fix: weight the most recent segment (bi→bi-1)
   //    twice as heavily as the older segment (bi-1→bi-2): slope = (2*(nd0-nd1) + (nd1-nd2)) / 3.
   double c6 = 0.0;
   if(bi >= 2)
     {
      ...
      // [V8-21] C6-SCALE: the old * 3.0 amplifier was calibrated for the 2-point formula
      //    whose max unscaled value was 1.0, giving effective amplification of 3.0 before
      //    clamping. The 3-bar formula's max unscaled value is 4/3, so the same * 3.0
      //    produced effective amplification of 4.0 — causing C6 to saturate at ±1.0 more
      //    readily. * 2.25 restores the original clamp-engagement frequency:
      //    (4/3) * 2.25 ≈ 3.0, matching the old formula's sensitivity range.
   ...
   if(bi >= 2)
     {
      ...
      double nd1 = (double)g_bars[bi-1].total_delta / v1;
      double nd2 = (double)g_bars[bi-2].total_delta / v2;
      if(MathIsValidNumber(nd0) && MathIsValidNumber(nd1) && MathIsValidNumber(nd2))
        {
         double slope = (2.0 * (nd0 - nd1) + (nd1 - nd2)) / 3.0;
         if(isBuy && slope > 0.1)
