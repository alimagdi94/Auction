//+------------------------------------------------------------------+
//|  OrderFlowEA_v812.mq5                                            |
//|  Order Flow EA — Production Grade v8.12                          |
//|                                                                    |
//|  v8.12 — March 2026 code-review patch (3 fixes applied).        |
//|  All v8.11 logic preserved; this release addresses:              |
//|                                                                    |
...
#property description "Order Flow EA v8.12 — Production-Grade (Code Review Fixes Applied)"
#property strict

// [V8-18] Single source of truth for the EA version string.
#define EA_VERSION "8.12"
   ...
   if(!g_equityHalted && InpMaxEquityProfit > 0.0 && equity >= balance + InpMaxEquityProfit)
     {
      ...
      Alert(EA_NAME " — EQUITY PROFIT TARGET HIT (+"
            + DoubleToString(InpMaxEquityProfit, 2) + "). Auto-trading halted. [V8-26]");
   ...
   if(!g_equityHalted && InpMaxEquityLoss > 0.0 && equity <= balance - InpMaxEquityLoss)
     {
      ...
      Alert(EA_NAME " — EQUITY LOSS LIMIT HIT (-"
            + DoubleToString(InpMaxEquityLoss, 2) + "). Auto-trading halted. [V8-26]");
   ...
   for(int attempt = 1; attempt <= 3; attempt++)
     {
      ...
      //    Sleep() blocks the entire EA tick thread; 400 ms total across two retries
      //    prevents ManagePositions() from updating trailing stops during exactly
      //    the fast-market conditions (news spikes) that cause requotes.
      //    50 ms is sufficient for a broker price refresh without materially
      //    delaying position management.
