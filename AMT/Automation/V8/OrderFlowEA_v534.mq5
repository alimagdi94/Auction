...
void RefreshSymbolInfo()
  {
   ...
   // Odd digit count means sub-pip pricing (3,5,7...) — pip = 10 × _Point.
   // Even digit count (0,2,4,6) — pip equals _Point directly.
   ...
   if(g_TickSize <= 0.0)
     ...
     {
      // Use already-fetched locals — avoids two extra broker API calls per bar.
      LogSystem(StringFormat(
         "OrderFlowEA — SymbolInfo | Symbol: %s | tvBase=%.6f tvSize=%.6f _Point=%.6f"
         " => g_TickSize=%.6f ($/point/lot) | g_Pip=%.6f | digits=%d",
         _Symbol, tvBase, tvSize, _Point, g_TickSize, g_Pip, digits));
     }
  }
...
void PlaceOrders()
  {
   ...
   // Guard: degenerate bar (no tick data yet) — skip this bar entirely.
