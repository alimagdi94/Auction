...
#property description "Footprint Chart EA v5.30 — Full footprint + Discord + Automated Trading"
#property strict
...
bool trade_Send(ENUM_TRADE_REQUEST_ACTIONS action,
                ...
                string                     comment)
  {
   ...
   req.magic        = g_Magic;
   req.comment      = comment;
   req.type_filling = GetBrokerFillingMode(); // broker-safe filling mode (was hardcoded IOC)
   req.expiration   = 0;

   // Safety: ensure SL/TP are not inside the broker's minimum stop distance
   if(sl > 0.0)
     {
      ...
      if(slDist < minDist)
        {
         if(orderType == ORDER_TYPE_BUY_STOP || orderType == ORDER_TYPE_BUY)
            req.sl = NormalizeDouble(price - minDist, _Digits);
         else
            req.sl = NormalizeDouble(price + minDist, _Digits);
        }
     }
   if(tp > 0.0)
     {
      ...
      if(tpDist < minDist)
        {
         if(orderType == ORDER_TYPE_BUY_STOP || orderType == ORDER_TYPE_BUY)
            req.tp = NormalizeDouble(price + minDist, _Digits);
         else
            req.tp = NormalizeDouble(price - minDist, _Digits);
        }
     }

   // Retry loop: handles transient requote/busy retcodes (max 3 attempts)
   bool ok = false;
   for(int attempt = 1; attempt <= 3; attempt++)
     {
      ...
      if(ok) break;
      uint rc = res.retcode;
      // Retry only on requote or server busy — all other errors are fatal
...
//+------------------------------------------------------------------+
//| PlaceOrders — evaluate last closed bar and fire pending orders   |
//+------------------------------------------------------------------+
void PlaceOrders()
  {
   if(!g_autoTrade)
   ...
   // --- Position limit: skip if already at or above the allowed position count ---
   ...
   // --- Get last closed bar signal from footprint data ---
   int nBars = ArraySize(g_bars);
   if(nBars < 2)
      return;

   int bi = nBars - 2;  // last closed bar (index n-1 is the forming bar)
   if(g_bars[bi].level_count == 0 || g_bars[bi].total_vol == 0)
      return;
   if(!g_bars[bi].sorted)
      ComputeBarSignals(bi);
   ...
   // Clean stale pending orders before placing new ones
