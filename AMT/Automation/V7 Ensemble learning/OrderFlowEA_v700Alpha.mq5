//+------------------------------------------------------------------+
//|  OrderFlowEA_v700.mq5                                            |
//|  Order Flow EA — Production Grade v7.00                          |
//|                                                                    |
//|  Upgraded from v6.05. All enhancements derived from the           |
//|  comprehensive system review (March 2026). Core footprint          |
//|  order-flow concept preserved; risk model, signal quality,         |
//|  and architecture upgraded to institutional standards.             |
//|                                                                    |
//|  Changes from v6.05:                                               |
//|                                                                    |
//|  [V7-01] TRUE RISK-BASED LOT SIZING                               |
//|    CalcLot() now uses lot = RiskAmt / (SLpoints × PointValue).    |
//|    Margin-based approach retained as fallback only. This ensures   |
//|    exactly InpRiskPercent% of balance is at risk on every trade    |
//|    regardless of instrument leverage.                              |
//|                                                                    |
//|  [V7-02] HTF TREND FILTER                                         |
//|    Optional H1 EMA filter (CheckHTFTrend). BUY only when price    |
//|    > HTF EMA; SELL only when price < HTF EMA. Eliminates the      |
//|    dominant class of counter-trend entries that caused 90%+        |
//|    stop-out rate in the backtest.                                  |
//|                                                                    |
//|  [V7-03] SESSION TIME FILTER                                      |
//|    CheckSessionTime() blocks entries outside configurable window   |
//|    (default 07:00–17:00 UTC). Eliminates low-liquidity Asian-      |
//|    session noise signals where footprint data is unreliable.       |
//|                                                                    |
//|  [V7-04] DAILY LOSS LIMIT                                         |
//|    CheckDailyLoss() tracks equity vs. day-start balance.           |
//|    Halts auto-trading when daily loss exceeds InpMaxDailyLoss%.    |
//|    Resets automatically at the start of each new trading day.     |
//|                                                                    |
//|  [V7-05] CONSECUTIVE LOSS PROTECTION                              |
//|    OnTradeTransaction() now tracks consecutive SL hits.            |
//|    After InpMaxConsecLosses hits: 50% size reduction for           |
//|    InpSizeReductionTrades trades.                                  |
//|    After InpHaltConsecLosses hits: full session halt + alert.      |
//|    Reset on any TP hit or positive net PnL close.                  |
//|                                                                    |
//|  [V7-06] ADAPTIVE SIGNAL THRESHOLD                                |
//|    ComputeAdaptiveThreshold() scales the HFT threshold with        |
//|    ATR volatility vs. a stored baseline. High-vol environments     |
//|    demand a stronger signal; low-vol environments accept a softer  |
//|    signal. Clamped to [InpAdaptiveThreshMin, InpAdaptiveThreshMax].|
//|                                                                    |
//|  [V7-07] CONVICTION DIVERSITY GATE                                |
//|    Requires InpMinConvictionComp distinct conviction components    |
//|    before a trade fires. Eliminates the "AbsLow monopoly" where    |
//|    a single footprint pattern drove 100% of entries.               |
//|                                                                    |
//|  [V7-08] SL COOLDOWN (PER DIRECTION)                              |
//|    After a stop-loss hit, blocks new entries in the same           |
//|    direction for InpSLCooldownBars bars. Prevents the chasing      |
//|    pattern of multiple SL hits in rapid succession.                |
//|                                                                    |
//|  [V7-09] ABSORPTION VOLUME GATE                                   |
//|    Absorption signals now require volume > InpAbsorptionRatio ×    |
//|    avgVol AND the absorbing side volume must be ≥ InpAbsVolMult ×  |
//|    the opposing side at that level. Reduces false absorption flags. |
//|                                                                    |
//|  [V7-10] ATR-RELATIVE SPREAD CHECK                                |
//|    Spread filter now dual-checks: absolute pips AND ATR-relative   |
//|    ratio (spread > ATR × InpSpreadATRRatio). Blocks news spikes    |
//|    that the fixed-pips check misses during volatility expansion.   |
//|                                                                    |
//|  [V7-11] PENDING ORDER STALENESS TIMEOUT                          |
//|    ManagePositions() now deletes unfilled pending orders older      |
//|    than InpPendingExpiryBars bars. Prevents stale orders from      |
//|    filling far from their signal context.                           |
//|                                                                    |
//|  [V7-12] RECALIBRATED HFT WEIGHTS                                 |
//|    C4 (Absorption at extremes): 15% → 10%                         |
//|    C6 (3-bar CVD momentum): 10% → 15%                             |
//|    Reduces over-contribution of the absorption component that      |
//|    caused the 100% AbsLow conviction pattern in the backtest.      |
//|                                                                    |
//|  [V7-13] MODULAR PlaceOrders / CONVICTIONRESULT STRUCT            |
//|    PlaceOrders() decomposed into CheckRiskConditions(),            |
//|    CheckSessionTime(), CheckHTFTrend(), EvaluateBarSignal(),       |
//|    and SubmitEntry(). ConvictionResult struct carries component     |
//|    count alongside the label for the diversity gate.               |
//|                                                                    |
//|  [V7-14] RECALIBRATED DEFAULT PARAMETERS                          |
//|    InpSignalThreshold: 60 (base for adaptive scaling)             |
//|    InpSLMode: ATR (was Bar — adapts to volatility)                |
//|    InpSLATRMult: 1.5 (was 15.0)                                   |
//|    InpRiskRewardRatio: 2.0 (was 60.0)                             |
//|    InpBreakEvenTrigger: 15.0 pips (was 600.0)                     |
//|    InpBreakEvenBuffer: 2.0 pips (was 200.0)                       |
//|    InpTrailStart: 20.0 pips (was 1200.0)                          |
//|    InpTrailStep: 5.0 pips (was 300.0)                             |
//|    InpBufferPips: 3.0 (was 200.0)                                  |
//|    InpSpreadFilter: true (was false)                               |
//|    InpMaxEquityLoss: 1500.0 (was 0.0 / disabled)                  |
//|    InpMaxEquityProfit: 3000.0 (was 0.0 / disabled)                |
//|                                                                    |
//|  Inherited fixes from v6.05 (all preserved):                       |
//|    FIX-DRAW, FIX-SELL-1..4, FIX-1..11, ANALYSIS-MODE             |
//+------------------------------------------------------------------+
#property copyright   "Ali Magdy"
#property version     "7.00"
#property description "Order Flow EA v7.00 — Production-Grade Upgrade"
#property strict
...
input group "Logging"
input bool          InpLoggingEnable = true;
input ENUM_LOG_MODE InpLogMode       = LOG_FULL;
...
input double InpAbsVolMult      = 2.0;   // [V7-09] Absorbing side must be >= X× opposing side
input int    InpHistoryBars     = 200;
input double InpVAPercent       = 70.0;
input double InpHVNRatio        = 2.0;
input double InpLVNRatio        = 0.35;

input group "Aggregation"
input ENUM_FOOT_CHART_MODE InpChartMode      = FOOT_CHART_DELTA;
input int                  InpTickMultiplier = 5;

input group "Bid/Ask Exhaustion Signal"
input bool   InpExhaustionEnable  = true;
input int    InpExhaustionCells   = 3;
input double InpExhaustionZeroRat = 0.05;

input group "Order Flow Strength Score"
input double InpOFWtDelta         = 40.0;
input double InpOFWtImb           = 25.0;
input double InpOFWtStacked       = 20.0;
input double InpOFWtAbsorb        = 15.0;

input group "High Probability Signals"
input bool   InpShowSignals       = false;                 // Matches benchmark .set
input int    InpSignalThreshold   = 35;                    // Matches benchmark .set
input int    InpSignalFreqBars    = 3;                     // Matches benchmark .set
input string InpSignalBuySound    = "";                    // Matches benchmark .set
input string InpSignalSellSound   = "";                    // Matches benchmark .set

input group "V7 — Signal Quality Filters (all OFF = benchmark-compatible)"
input int    InpMinConvictionComp = 0;    // [V7-07] 0 = disabled; set >= 2 to enable diversity gate
input bool   InpAdaptiveThreshold = false; // [V7-06] false = use InpSignalThreshold directly
input double InpAdaptiveThreshMin = 35.0;  // [V7-06] Floor (only used when adaptive is ON)
input double InpAdaptiveThreshMax = 75.0;  // [V7-06] Ceiling (only used when adaptive is ON)
input int    InpSLCooldownBars    = 0;    // [V7-08] 0 = disabled; set > 0 to block re-entry after SL

input group "V7 — Higher-Timeframe Trend Filter (OFF = benchmark-compatible)"
input bool            InpHTFEnable  = false;               // [V7-02] false = no HTF check
input ENUM_TIMEFRAMES InpHTFPeriod  = PERIOD_H1;
input int             InpHTFEMA     = 50;

input group "V7 — Session Time Filter (OFF = benchmark-compatible)"
input bool   InpSessionEnable    = false;                  // [V7-03] false = trade any hour
input int    InpSessionStartHour = 7;                      // UTC server hour (inclusive)
input int    InpSessionEndHour   = 17;                     // UTC server hour (exclusive)

input group "Automated Trading — Strategy"
input bool          InpAnalysisMode      = false;
input bool          InpATEnable          = true;           // Matches benchmark .set
input ENUM_ORDER_MODE InpOrderMode       = ORDER_MODE_MARKET;
input int           InpATR_Period        = 14;
input bool          InpSpreadFilter      = false;          // Matches benchmark .set
input double        InpMaxSpread         = 3.0;
input double        InpSpreadATRRatio    = 0.0;            // [V7-10] 0 = disabled
input bool          InpAllowBuy          = true;
input bool          InpAllowSell         = true;
input double        InpBufferPips        = 200.0;          // Matches benchmark .set
input int           InpPendingExpiryBars = 0;              // [V7-11] 0 = disabled

input group "Automated Trading — Money Management"
input bool   InpUseRiskPercent    = true;
input double InpRiskPercent       = 1.0;
input double InpFixedLot          = 0.1;                   // Matches benchmark .set

input group "Automated Trading — Exit"
input bool          InpUseStopLoss      = true;
input ENUM_SL_MODE  InpSLMode           = SL_MODE_BAR;    // Matches benchmark .set (mode 0)
input double        InpSLPips           = 200.0;           // Matches benchmark .set
input double        InpSLATRMult        = 15.0;            // Matches benchmark .set

input bool          InpUseTakeProfit    = true;
input ENUM_TP_MODE  InpTPMode           = TP_MODE_RR;
input double        InpRiskRewardRatio  = 60.0;            // Matches benchmark .set
input double        InpTPPips           = 40.0;
input double        InpTPATRMult        = 30.0;            // Matches benchmark .set

input group "Automated Trading — The Guardian"
input bool   InpUseBreakEven      = true;
input double InpBreakEvenTrigger  = 600.0;                 // Matches benchmark .set
input double InpBreakEvenBuffer   = 200.0;                 // Matches benchmark .set
input bool   InpUseTrailing       = true;
input double InpTrailStart        = 1200.0;                // Matches benchmark .set
input double InpTrailStep         = 300.0;                 // Matches benchmark .set

input group "Automated Trading — Account Safety"
input double InpMaxEquityProfit      = 0.0;   // Matches benchmark .set (0 = disabled)
input double InpMaxEquityLoss        = 0.0;   // Matches benchmark .set (0 = disabled)
input double InpMaxDailyLossPercent  = 0.0;   // [V7-04] 0 = disabled; set e.g. 3.0 to enable
input int    InpMaxConsecLosses      = 0;     // [V7-05] 0 = disabled; set e.g. 5 to enable size reduction
input int    InpHaltConsecLosses     = 0;     // [V7-05] 0 = disabled; set e.g. 10 to enable halt
input int    InpSizeReductionTrades  = 3;     // [V7-05] Trades at reduced size when limit triggered
input bool   InpCleanOldOrders    = true;
input int    InpMaxPositions      = 1;
input ulong  InpMagic             = 20260226;

input group "Visuals"
input bool  InpShowVisuals     = true;
input bool  InpShowSLTPLines   = true;
input bool  InpShowEntryLabel  = true;
input bool  InpShowExitLabel   = true;
input color InpVizBuyColor     = clrDodgerBlue;
input color InpVizSellColor    = clrOrangeRed;
input color InpVizSLColor      = clrFireBrick;
input color InpVizTPColor      = clrMediumSeaGreen;
...
ConvictionResult GetConvictionResult(int bi, bool isBuy)
  {
   ...
   if(g_bars[bi].is_delta_divergence)
     {
      int n = ArraySize(tags); ArrayResize(tags, n+1);
      tags[n] = "DeltaDiv";
     }
   ...
   if(g_bars[bi].total_vol > 0)
     {
      ...
      if(isBuy && dr > 0.35)
        { int n = ArraySize(tags); ArrayResize(tags,n+1); tags[n] = "BullDelta"; }
      else if(!isBuy && dr < -0.35)
        { int n = ArraySize(tags); ArrayResize(tags,n+1); tags[n] = "BearDelta"; }
     }
   ...
   if(isBuy)
     {
      if(hasSB)
        { int n = ArraySize(tags); ArrayResize(tags,n+1); tags[n] = "StackBuy"; }
      if(absLow)
        { int n = ArraySize(tags); ArrayResize(tags,n+1); tags[n] = "AbsLow"; }
      if(exhBid)
        { int n = ArraySize(tags); ArrayResize(tags,n+1); tags[n] = "BidExh"; }
     }
   else
     {
      if(hasSS)
        { int n = ArraySize(tags); ArrayResize(tags,n+1); tags[n] = "StackSell"; }
      if(absHigh)
        { int n = ArraySize(tags); ArrayResize(tags,n+1); tags[n] = "AbsHigh"; }
      if(exhAsk)
        { int n = ArraySize(tags); ArrayResize(tags,n+1); tags[n] = "AskExh"; }
     }

   // Component 4: CVD momentum (use C6 sign)
   if(bi >= 2)
     {
      long v0 = MathMax(1, g_bars[bi].total_vol);
      long v2 = MathMax(1, g_bars[bi-2].total_vol);
      double nd0 = (double)g_bars[bi].total_delta   / v0;
      double nd2 = (double)g_bars[bi-2].total_delta / v2;
      if(MathIsValidNumber(nd0) && MathIsValidNumber(nd2))
        {
         double slope = (nd0 - nd2) / 2.0;
         if(isBuy && slope > 0.1)
           { int n = ArraySize(tags); ArrayResize(tags,n+1); tags[n] = "CVD+"; }
   ...
   if(g_bars[bi].is_naked_poc)
     { int n = ArraySize(tags); ArrayResize(tags,n+1); tags[n] = "NakedPOC"; }
   ...
   // Count distinct signal components (NakedPOC is context, not a signal component)
   res.componentCount = 0;
   for(int i = 0; i < total; i++)
      if(tags[i] != "NakedPOC") res.componentCount++;
   ...
   if(loaded > 0)
     {
      ...
      LogSystem(StringFormat("OrderFlowEA v7.00 — History: %d ticks, %d bars",
...
bool CheckHTFTrend(bool isBuy)
  {
   ...
   if(!aligned)
      LogTradeExec(StringFormat(
         "CheckHTFTrend: %s blocked — price %.5f %s HTF EMA(%.0f) %.5f",
         isBuy?"BUY":"SELL", curPrice, isBuy?"<=":">", (double)InpHTFEMA, emaVal));
   return aligned;
  }

// [V7-04] Returns true if daily loss limit has not been breached.
//    Resets day-start balance automatically on each new calendar day.
bool CheckDailyLoss()
  {
   ...
   if(dt.day != g_dayStartDay)
     {
      ...
      LogRisk(StringFormat("New day — DayStartBalance: %.2f | DailyLossLimit: %.1f%%",
                           g_dayStartBalance, InpMaxDailyLossPercent));
     }
   ...
   if(g_dayStartBalance - equity >= lossLimit)
     {
      ...
      Alert(StringFormat("OrderFlowEA v7.00 — DAILY LOSS LIMIT REACHED (%.1f%%). Halted until next day.", InpMaxDailyLossPercent));
...
void EvalAndFireSignal()
  {
   ...
   LogSignal(StringFormat(
      "OrderFlowEA v7.00 — %s | %s %s | HFT: %d (thresh=%d) | OFS: %d"
...
ENUM_ORDER_TYPE_FILLING GetBrokerFillingMode()
  {
   ...
   if(!s_warned) { LogWarning(StringFormat("Filling flags=0x%X unrecognised; defaulting RETURN.", flags)); s_warned=true; }
...
int OnInit()
  {
   // --- Input validation ---
   if(_Point <= 0.0)
     { Alert("OrderFlowEA v7.00: Invalid symbol point size."); return INIT_FAILED; }
   if(InpTickSize <= 0)
   ...
   LogSystem(StringFormat(
      "OrderFlowEA v7.00 — Init | %s | TickSize: %dpts×%d | HistBars: %d"
      " | AutoTrade: %s | AnalysisMode: %s | HasTrades: %s"
      " | HTFFilter: %s (%s EMA%d) | Session: %s (%02d:00-%02d:00 UTC)"
...
void OnDeinit(const int reason)
  {
   ...
   LogSystem(StringFormat("OrderFlowEA v7.00 — Deinit (reason=%d)", reason));
  }
...
void OnTradeTransaction(const MqlTradeTransaction &trans,
                        ...
                        const MqlTradeResult      &result)
  {
   ...
   // [V7-05] Determine if this was a SL hit from the comment
   bool   isSLHit = (StringFind(comment, "sl") >= 0 && StringFind(comment, "tp") < 0);
   bool   isTPHit = (StringFind(comment, "tp") >= 0);
      ...
      if(InpHaltConsecLosses > 0 && g_consecutiveLosses >= InpHaltConsecLosses && !g_sessionHalted)
        {
         ...
         Alert(StringFormat(
            "OrderFlowEA v7.00 — SESSION HALTED: %d consecutive SL hits reached (%d limit).",
