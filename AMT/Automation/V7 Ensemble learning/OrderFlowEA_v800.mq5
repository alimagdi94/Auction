//+------------------------------------------------------------------+
//|  OrderFlowEA_v800.mq5                                            |
//|  Order Flow EA — Production Grade v8.00                          |
//|                                                                    |
//|  Code review & production upgrade from v7.00 (March 2026).        |
//|  All v7.xx logic fully preserved; this release addresses:         |
//|                                                                    |
//|  [V8-01] DESCRIPTIVE INPUT PARAMETERS                             |
//|    Every input now carries a meaningful inline description          |
//|    visible in the MT5 Strategy Tester and Inputs dialog.          |
//|    Replaced all "Matches benchmark .set" placeholders.             |
//|                                                                    |
//|  [V8-02] CALIBRATED DEFAULT VALUES                                |
//|    Defaults corrected to the v7.00 target specification            |
//|    (changelog V7-14). Previous defaults were left at legacy        |
//|    v6.05 benchmark values for comparison testing but were          |
//|    never updated for production use.                               |
//|      InpSignalThreshold  : 60   (was 35)                          |
//|      InpSLMode           : ATR  (was Bar)                         |
//|      InpSLATRMult        : 1.5  (was 15.0)                        |
//|      InpSLPips           : 20.0 (was 200.0)                       |
//|      InpRiskRewardRatio  : 2.0  (was 60.0)                        |
//|      InpTPATRMult        : 3.0  (was 30.0)                        |
//|      InpBreakEvenTrigger : 15.0 (was 600.0)                       |
//|      InpBreakEvenBuffer  : 2.0  (was 200.0)                       |
//|      InpTrailStart       : 20.0 (was 1200.0)                      |
//|      InpTrailStep        : 5.0  (was 300.0)                       |
//|      InpBufferPips       : 3.0  (was 200.0)                       |
//|      InpSpreadFilter     : true (was false)                        |
//|      InpMaxEquityLoss    : 1500.0 (was 0.0)                       |
//|      InpMaxEquityProfit  : 3000.0 (was 0.0)                       |
//|                                                                    |
//|  [V8-03] BROKER FILLING MODE — DEFENSIVE COMMENT                  |
//|    GetBrokerFillingMode() removes the erroneous SYMBOL_FILLING_    |
//|    RETURN bitmask check (that constant does not exist in MQL5).    |
//|    RETURN is correctly applied as the unconditional fallback when   |
//|    neither FOK nor IOC is set in SYMBOL_FILLING_MODE.              |
//|                                                                    |
//|  [V8-04] SL HIT DETECTION — DEFENSIVE FALLBACK                    |
//|    OnTradeTransaction() comment-based SL detection now also        |
//|    checks dealNet < 0 as a fallback when the broker strips or      |
//|    truncates order comments, preventing missed loss streaks.        |
//|                                                                    |
//|  [V8-05] DAILY LOSS RESET — EQUITY vs BALANCE GUARD               |
//|    CheckDailyLoss() now snapshots balance at the first tick of     |
//|    the new day only after pending close orders settle, avoiding     |
//|    a stale snapshot when an overnight position closes at open.      |
//|                                                                    |
...
#property description "Order Flow EA v8.00 — Production-Grade"
#property strict
...
input int    InpSessionStartHour = 7;     // Session open — UTC server hour, inclusive (e.g. 7 = 07:00)
input int    InpSessionEndHour   = 17;    // Session close — UTC server hour, exclusive (e.g. 17 = up to 16:59)
   ...
   if(loaded > 0)
     {
      ...
      LogSystem(StringFormat("OrderFlowEA v8.00 — History: %d ticks, %d bars",
   ...
   if(g_dayStartBalance - equity >= lossLimit)
     {
      ...
      Alert(StringFormat("OrderFlowEA v8.00 — DAILY LOSS LIMIT REACHED (%.1f%%). Halted until next day.", InpMaxDailyLossPercent));
...
void EvalAndFireSignal()
  {
   ...
   LogSignal(StringFormat(
      "OrderFlowEA v8.00 — %s | %s %s | HFT: %d (thresh=%d) | OFS: %d"
...
int OnInit()
  {
   // --- Input validation ---
   if(_Point <= 0.0)
     { Alert("OrderFlowEA v8.00: Invalid symbol point size."); return INIT_FAILED; }
   if(InpTickSize <= 0)
   ...
   LogSystem(StringFormat(
      "OrderFlowEA v8.00 — Init | %s | TickSize: %dpts×%d | HistBars: %d"
...
void OnDeinit(const int reason)
  {
   ...
   LogSystem(StringFormat("OrderFlowEA v8.00 — Deinit (reason=%d)", reason));
  }
...
void OnTradeTransaction(const MqlTradeTransaction &trans,
                        ...
                        const MqlTradeResult      &result)
  {
   ...
   // [V8-04] SL hit detection: comment-based primary check, net-loss as fallback.
   //    Some brokers strip or truncate comments — the fallback catches those cases.
      ...
      if(InpHaltConsecLosses > 0 && g_consecutiveLosses >= InpHaltConsecLosses && !g_sessionHalted)
        {
         ...
         Alert(StringFormat(
            "OrderFlowEA v8.00 — SESSION HALTED: %d consecutive SL hits reached (%d limit).",
