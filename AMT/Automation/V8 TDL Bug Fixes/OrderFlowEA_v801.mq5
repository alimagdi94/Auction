//+------------------------------------------------------------------+
//|  OrderFlowEA_v801.mq5                                            |
//|  Order Flow EA — Production Grade v8.01                          |
//|                                                                    |
//|  v8.01 — March 2026 production-readiness patch (TDL applied).    |
//|  All v8.00 logic preserved; this release addresses:              |
//|                                                                    |
...
#property description "Order Flow EA v8.01 — Production-Grade (TDL Applied)"
#property strict
...
int OnInit()
  {
   ...
   LogSystem(StringFormat(
      "OrderFlowEA v8.01 — Init | %s | TickSize: %dpts×%d | HistBars: %d"
...
void OnDeinit(const int reason)
  {
   ...
   LogSystem(StringFormat("OrderFlowEA v8.01 — Deinit (reason=%d)", reason));
  }
...
void OnTick()
  {
   ...
   if(!g_hasTrades && SymbolInfoDouble(_Symbol, SYMBOL_LAST) > 0.0)
     {
      g_hasTrades = true;
      LogSystem("g_hasTrades re-checked: LAST price now available — switching to tick-flag classification. [V8-13]");
     }
      ...
      if(InpHaltConsecLosses > 0 && g_consecutiveLosses >= InpHaltConsecLosses && !g_sessionHalted)
        {
         ...
         Alert(StringFormat(
            "OrderFlowEA v8.01 — SESSION HALTED: %d consecutive SL hits reached (%d limit).",
