//+------------------------------------------------------------------+
//|  OrderFlowEA_v802.mq5                                            |
//|  Order Flow EA — Production Grade v8.02                          |
//|                                                                    |
//|  v8.02 — March 2026 code-review patch (6 fixes applied).        |
//|  All v8.01 logic preserved; this release addresses:              |
//|                                                                    |
//|  [V8-17] POC GRAVITY C3 SIGNAL INVERTED (BUG 1)                 |
...
#property description "Order Flow EA v8.02 — Production-Grade (Code Review Fixes Applied)"
#property strict
   ...
   if(loaded > 0)
     {
      ...
      LogSystem(StringFormat("OrderFlowEA v8.01 — History: %d ticks, %d bars",
      ...
      if(!positionsOpen)
        {
         ...
         RiskStateSave();   // [V8-10]
         LogRisk(StringFormat("New day — DayStartBalance: %.2f | DailyLossLimit: %.1f%% [V8-07 settled]",
   ...
   if(g_dayStartBalance - equity >= lossLimit)
     {
      ...
      RiskStateSave();   // [V8-10]
      Alert(StringFormat("OrderFlowEA v8.01 — DAILY LOSS LIMIT REACHED (%.1f%%). Halted until next day.", InpMaxDailyLossPercent));
...
void EvalAndFireSignal()
  {
   ...
   LogSignal(StringFormat(
      "OrderFlowEA v8.01 — %s | %s %s | HFT: %d (thresh=%d) | OFS: %d"
...
int OnInit()
  {
   // --- Input validation ---
   if(_Point <= 0.0)
     { Alert("OrderFlowEA v8.01: Invalid symbol point size."); return INIT_FAILED; }
   if(InpTickSize <= 0)
   ...
   if(InpATEnable && InpAnalysisMode)
      Alert("OrderFlowEA v8.01: InpATEnable=true but InpAnalysisMode=true — no real orders will be placed. [V8-12]");
   ...
   if(InpATEnable && !InpAnalysisMode &&
      ...
      Alert("OrderFlowEA v8.02 [V8-17]: WARNING — All loss-protection limits are disabled "
   ...
   LogSystem(StringFormat(
      "OrderFlowEA v8.02 — Init | %s | TickSize: %dpts×%d | HistBars: %d"
...
void OnDeinit(const int reason)
  {
   ...
   LogSystem(StringFormat("OrderFlowEA v8.02 — Deinit (reason=%d)", reason));
  }
      ...
      if(InpHaltConsecLosses > 0 && g_consecutiveLosses >= InpHaltConsecLosses && !g_sessionHalted)
        {
         ...
         Alert(StringFormat(
            "OrderFlowEA v8.02 — SESSION HALTED: %d consecutive SL hits reached (%d limit).",
