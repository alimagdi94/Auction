//+------------------------------------------------------------------+
//|  OrderFlowEA_v819.mq5                                            |
//|  Order Flow EA — Production Grade v8.19                          |
//|                                                                    |
...
#property description "Order Flow EA v8.19 — Production-Grade (Code Review Fixes Applied)"
#property strict

// [V8-18] Single source of truth for the EA version string.
#define EA_VERSION "8.19"
   ...
   for(int attempt = 1; attempt <= 3; attempt++)
     {
      if(attempt > 1 && action == TRADE_ACTION_DEAL)
        {
         // [V8-19] RETRY-DUPE-GUARD: before retrying a market order after a
         //    timeout/connection failure, check whether the original send already
         //    filled on the broker side. MT5 delivers ORDER/DEAL events via
         //    OnTradeTransaction asynchronously, so a filled order may not yet
         //    appear in PositionsTotal(). We search both the live position pool and
         //    the recent deal history (last 60 s) for a fill that matches our magic,
         //    symbol, and volume. If found, treat the send as successful so we do
         //    not place a duplicate entry.
         ...
         for(int pi = PositionsTotal()-1; pi >= 0 && !alreadyFilled; pi--)
           {
            ...
            if((ulong)PositionGetInteger(POSITION_MAGIC) != g_Magic) continue;
            if(PositionGetString(POSITION_SYMBOL)        != _Symbol)  continue;
            if(MathAbs(PositionGetDouble(POSITION_VOLUME) - req.volume) < 1e-8)
              { alreadyFilled = true; outTicket = ptk; }
           }
         // 2. Recent deal history (covers the window between fill and position update)
         if(!alreadyFilled &&
            ...
            for(int di = dTotal-1; di >= 0 && !alreadyFilled; di--)
              {
               ...
               if(MathAbs(HistoryDealGetDouble(dTk, DEAL_VOLUME) - req.volume) < 1e-8)
                 {
                  alreadyFilled = true;
         ...
         if(alreadyFilled)
           {
            LogTradeExec(StringFormat(
               "RETRY-DUPE-GUARD: prior attempt filled (ticket=%I64u). Aborting retry. [V8-19]",
               outTicket));
            ok = true;
            break;
           }
      ...
      for(int oi = OrdersTotal()-1; oi >= 0; oi--)
        {
         ...
         LogTradeExec(StringFormat(
            "PENDING-EXPIRY-RESTART: re-registered ticket=%I64u placed=%s",
