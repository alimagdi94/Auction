...
void DrawBar(int bi, int shift, int barW)
{
   ...
   for(int i = 0; i < len; i++)
   {
      ...
      // Normal Neon Heatmap Backgrounds (Muted Priority)
      color bidCol = LerpColor(InpBidBaseColor, InpBidHighColor, intensity);
      color askCol = LerpColor(InpAskBaseColor, InpAskHighColor, intensity);
      ...
      if(!skipText)
      {
         ...
         // Force dark text if intensity extremely bright to not lose contrast, though colors are muted now.
         if(intensity > 0.85 && inVA) { tCBid = colBlackText; tCAsk = colBlackText; }
