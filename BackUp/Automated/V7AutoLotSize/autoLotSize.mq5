...
void RefreshSymbolInfo()
  {
   ...
   // g_TickSize = account-currency value of 1 POINT move per 1 lot
   // tvBase = value of 1 tick (SYMBOL_TRADE_TICK_VALUE)
   // tvSize = price size of 1 tick (SYMBOL_TRADE_TICK_SIZE)
   // $/point = tvBase * (_Point / tvSize)
   // Works universally: FX (tvSize==_Point, ratio=1), JPY, Gold, CFDs
   g_TickSize = (tvSize > 0.0) ? tvBase * (_Point / tvSize) : 0.0;
   if(g_TickSize <= 0.0)
     {
      LogWarning(StringFormat(
         "OrderFlowEA — Tick value is zero for %s. Dynamic sizing disabled, using fixed lot.",
         _Symbol));
      g_TickSize = 0.0;
     }
   else
     {
      LogSystem(StringFormat(
         "OrderFlowEA — SymbolInfo | tvBase=%.6f tvSize=%.6f _Point=%.6f"
         " => g_TickSize=%.6f ($/point/lot) | g_Pip=%.6f",
         SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_VALUE),
         SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE),
         _Point, g_TickSize, g_Pip));
     }
  }
...
double CalcLot(double slPoints)
  {
   double lot;
   if(InpUseRiskPercent && g_TickSize > 0.0)
     {
      // If SL distance is zero (SL disabled or degenerate bar), fall back to InpSLPips
      double effPoints = (slPoints > 0.0) ? slPoints : (InpSLPips * g_Pip / _Point);
      double balance = AccountInfoDouble(ACCOUNT_BALANCE);
      double riskAmt = balance * InpRiskPercent / 100.0;
      lot = riskAmt / (effPoints * g_TickSize);
      LogTradeExec(StringFormat(
         "OrderFlowEA — Dynamic lot | Balance: %.2f | Risk: %.2f%% | RiskAmt: %.2f"
         " | SL_pts: %.1f | effPts: %.1f | TickSz($/pt): %.6f | Raw lot: %.4f",
         balance, InpRiskPercent, riskAmt, slPoints, effPoints, g_TickSize, lot));
     }
   else
      lot = InpFixedLot;
