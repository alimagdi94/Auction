//+------------------------------------------------------------------+
//|  OrderFlowEa_v540_Pure.mq5                                       |
//|  Order Flow EA — Pure Trading Engine v5.40                       |
//|  Footprint signals + chart markers + automated trading           |
//|  Production refactor — all 12 review findings resolved           |
//+------------------------------------------------------------------+
#property copyright   "Ali Magdy"
#property version     "5.40"
#property description "Order Flow EA v5.40 — Footprint signals + Automated Trading"
#property strict
...
// --- Precomputed OFS weights (set once in RefreshSymbolInfo) ---
// Avoids dividing by 100 and summing inputs on every ComputeOFScore call.
double g_wD = 0.40;   // OFS weight: delta ratio (normalised)
double g_wI = 0.25;   // OFS weight: imbalance count (normalised)
double g_wS = 0.20;   // OFS weight: stacked imbalance (normalised)
double g_wA = 0.15;   // OFS weight: absorption (normalised)
double g_wT = 1.00;   // Sum of all OFS weights (for renormalisation guard)

// --- OFScore single-bar memo: ComputeHFTSignal calls ComputeOFScore internally;
//     subsequent caller re-use hits the cache instead of doing a second scan. ---
int g_lastOFScoreBar   = -1;
int g_lastOFScoreValue = 50;

// --- Cached broker filling mode (refreshed at init + on new bar) ---
...
double GetEffectiveVAPercent()
  {
   // g_vaPercent is always set from InpVAPercent in OnInit; the fallback
   // chain is retained as a safety net for any future code path that calls
   // this before OnInit has run.
   return (g_vaPercent > 0.0) ? g_vaPercent : 70.0;
  }
...
//    Weights are precomputed in RefreshSymbolInfo and cached in g_wD/I/S/A/T.
//    Result is memoised per bar: ComputeHFTSignal calls this internally;
//    any subsequent same-bar call returns the cached value instantly.
int ComputeOFScore(int bi)
  {
   // Memo cache — valid until ReloadHistory invalidates it
   if(bi == g_lastOFScoreBar) return g_lastOFScoreValue;
   ...
   // Use precomputed, renormalised weights (see RefreshSymbolInfo)
   double raw   = (cDelta * g_wD + cImb * g_wI + cStack * g_wS + cAbsorb * g_wA) / g_wT;
   int    score = MathMax(0, MathMin(100, (int)(raw * 100.0 + 0.5)));

   g_lastOFScoreBar   = bi;
   g_lastOFScoreValue = score;
   return score;
  }
...
void ReloadHistory()
  {
   ...
   // Reset signal-evaluation cache — bar indices are rebuilt from scratch,
   // so any cached index is invalid and would cause EvalAndFireSignal to
   // silently skip signals until the cache naturally expires.
   g_sigCacheBarIdx   = -1;
   g_sigCacheVol      = -1;
   // Reset OFScore memo for the same reason.
   g_lastOFScoreBar   = -1;
   g_lastOFScoreValue = 50;
...
void SendDiscordAlert(bool isBuy, double hftScore, int ofsScore, const string conviction)
  {
...
void EvalAndFireSignal()
  {
   ...
   if(bi - g_lastSignalBar < g_signalFreqBars) return;
   ...
   SendDiscordAlert(isBuySignal, hftScore, currentScore, conviction);
  }
...
void RefreshSymbolInfo()
  {
   ...
   // Precompute OFS weights once here — ComputeOFScore uses g_wD/I/S/A/T
   // directly, avoiding per-call division and summation of four inputs.
   g_wD = MathMax(0.0, InpOFWtDelta)   / 100.0;
   g_wI = MathMax(0.0, InpOFWtImb)     / 100.0;
   g_wS = MathMax(0.0, InpOFWtStacked) / 100.0;
   g_wA = MathMax(0.0, InpOFWtAbsorb)  / 100.0;
   g_wT = g_wD + g_wI + g_wS + g_wA;
   if(g_wT <= 0.0) g_wT = 1.0;   // guard: user set all weights to 0

   // Cache broker filling mode — GetBrokerFillingMode called SymbolInfoInteger
   // on every order send; brokers never change this mid-session.
   g_FillingMode = GetBrokerFillingMode();
  }
...
bool trade_Send(ENUM_TRADE_REQUEST_ACTIONS action,
                ...
                ulong                     &outTicket)
  {
   ...
   req.magic        = g_Magic;
   req.comment      = comment;
   req.type_filling = g_FillingMode;   // cached at init — avoids SymbolInfoInteger per order
   req.expiration   = 0;
...
void PlaceOrders()
  {
   ...
   if(InpMaxPositions > 0 && CountOpenPositions() >= InpMaxPositions) return;
   ...
   // ComputeHFTSignal already called ComputeOFScore internally; the memo
   // cache returns bi's score instantly here — no second level-scan.
   int    ofsScore  = ComputeOFScore(bi);
   string conviction= GetConvictionReason(bi, direction);

   // Order comment: truncated to 31 chars for broker compatibility.
   // Full detail is in the journal via LogTradeExec below.
   string rawTag = StringFormat("FP.%s.%s.%d",
                                direction ? "B" : "S",
                                isMarket  ? "M" : "P",
                                hftInt);
   string tag = StringSubstr(rawTag, 0, 31);

   ulong ticket = 0;
   bool  sent   = trade_Send(action, orderType, entry, sl, tp, lot, tag, ticket);
            ...
            if(!modOk)
              {
               ...
               if(rc != TRADE_RETCODE_REQUOTE    &&
                  rc != TRADE_RETCODE_CONNECTION &&
                  rc != TRADE_RETCODE_TIMEOUT    &&
                  rc != TRADE_RETCODE_PRICE_OFF)   // 10021: transient broker code
                  break;
               Sleep(200);
              }
           }
...
int OnInit()
  {
   ...
   RefreshSymbolInfo();   // sets g_Pip, lot constraints, OFS weights, filling mode

   // Warn if all OFS weights are zero — ComputeOFScore will still run (guarded
   // by g_wT=1 fallback) but will always return 50 (neutral), disabling signals.
   if(InpOFWtDelta + InpOFWtImb + InpOFWtStacked + InpOFWtAbsorb <= 0.0)
      LogWarning("OrderFlowEA — All OFS weights are zero. Signals will always be neutral (score=50).");
   ...
   LogSystem(StringFormat(
      "OrderFlowEA — Initialised | Symbol: %s | TickSize: %dpts x%d | Step: %spts"
      " | HistoryBars: %d | AutoTrade: %s | HasTrades: %s"
      " | LogMode: %s",
...
void OnTick()
  {
   ...
   // now_msc uses +999 so CopyTicksRange includes all ticks that arrived
   // within the current second — without it, ticks in the last sub-second
   // fraction of the bar can be silently missed when TimeCurrent() is the
   // same value across consecutive OnTick calls within that second.
   MqlTick ticks[];
   uint    flag     = g_hasTrades ? COPY_TICKS_ALL : COPY_TICKS_INFO;
   long    now_msc  = (long)TimeCurrent() * 1000 + 999;
