//+------------------------------------------------------------------+
//|                                                    Footprint.mq5  |
//|   Footprint (Order Flow) — Production Ready v5.31                |
...
//|   v5.31 changes:                                                  |
//|   • All chart objects namespaced by ChartID — zero conflicts      |
//|     when attaching to any number of charts simultaneously         |
//|   • Market Order mode (TRADE_ACTION_DEAL) — instant fills at      |
//|     live ask/bid; Pending mode retained as option                 |
//|   • INVALID_PRICE eliminated: live tick refreshed before every    |
//|     OrderSend; pending entry validated vs current market          |
//|   • Detailed retcode logging (INVALID_PRICE, INVALID_STOPS etc.)  |
//+------------------------------------------------------------------+
#property copyright   "Ali Magdy"
#property version     "5.31"
#property description "Footprint Chart EA v5.31 — Full footprint + Discord + Automated Trading"
#property strict
...
input double InpRiskRewardRatio   = 2.0;          // TP = SL distance × this multiplier
...
double CalcLot(double slPoints)
  {
   ...
   if(InpUseRiskPercent && slPoints > 0.0 && g_TickSize > 0.0)
     ...
     {
      // Fallback: fixed lot when tick size is unavailable or risk sizing is disabled
      lot = InpFixedLot;
     }

   // Round to broker step and clamp
...
void PlaceOrders()
  {
   ...
   if(InpMaxEquityProfit > 0.0 && equity >= balance + InpMaxEquityProfit)
     {
      Print("Footprint EA — MaxEquityProfit reached. Auto-trading halted.");
      g_autoTrade = false;
      g_dirty     = true;
      return;
     }
   if(InpMaxEquityLoss > 0.0 && equity <= balance - InpMaxEquityLoss)
     {
      Print("Footprint EA — MaxEquityLoss hit. Auto-trading halted.");
      g_autoTrade = false;
      g_dirty     = true;
      return;
     }
   ...
   // --- Fetch live price for entry reference ---
   MqlTick lv;
   if(!SymbolInfoTick(_Symbol, lv))
     {
      Print("Footprint EA — PlaceOrders: SymbolInfoTick failed");
      return;
     }
   ...
   if(isBuy)
     {
      ...
      if(isMarket)
        {
         //--- Market BUY: entry = live ask, SL below bar low ---
         double entry = lv.ask;
         if(InpUseStopLoss)
            sl = NormalizeDouble(barLow - bufDist, _Digits);
         double slPoints = (sl > 0.0) ? MathAbs(entry - sl) / _Point : 0.0;
         double lot      = CalcLot(slPoints);
         if(InpUseTakeProfit && sl > 0.0)
            tp = NormalizeDouble(entry + (entry - sl) * InpRiskRewardRatio, _Digits);
         trade_Send(TRADE_ACTION_DEAL, ORDER_TYPE_BUY,
                    entry, sl, tp, lot,
                    StringFormat("FP_Buy_MKT%d", (int)MathRound(hftScore)));
        }
      else
        {
         //--- Pending BUY STOP: entry above bar high ---
         double entry = NormalizeDouble(barHigh + bufDist, _Digits);
         if(InpUseStopLoss)
            sl = NormalizeDouble(barLow - bufDist, _Digits);
         double slPoints = (sl > 0.0) ? MathAbs(entry - sl) / _Point : 0.0;
         double lot      = CalcLot(slPoints);
         if(InpUseTakeProfit && sl > 0.0)
            tp = NormalizeDouble(entry + (entry - sl) * InpRiskRewardRatio, _Digits);
         trade_Send(TRADE_ACTION_PENDING, ORDER_TYPE_BUY_STOP,
                    entry, sl, tp, lot,
                    StringFormat("FP_Buy_HFT%d", (int)MathRound(hftScore)));
        }
     }
   else  // isSell
     {
      ...
      if(isMarket)
        {
         //--- Market SELL: entry = live bid, SL above bar high ---
         double entry = lv.bid;
         if(InpUseStopLoss)
            sl = NormalizeDouble(barHigh + bufDist, _Digits);
         double slPoints = (sl > 0.0) ? MathAbs(sl - entry) / _Point : 0.0;
         double lot      = CalcLot(slPoints);
         if(InpUseTakeProfit && sl > 0.0)
            tp = NormalizeDouble(entry - (sl - entry) * InpRiskRewardRatio, _Digits);
         trade_Send(TRADE_ACTION_DEAL, ORDER_TYPE_SELL,
                    entry, sl, tp, lot,
                    StringFormat("FP_Sell_MKT%d", (int)MathRound(-hftScore)));
        }
      else
        {
         //--- Pending SELL STOP: entry below bar low ---
         double entry = NormalizeDouble(barLow - bufDist, _Digits);
         if(InpUseStopLoss)
            sl = NormalizeDouble(barHigh + bufDist, _Digits);
         double slPoints = (sl > 0.0) ? MathAbs(sl - entry) / _Point : 0.0;
         double lot      = CalcLot(slPoints);
         if(InpUseTakeProfit && sl > 0.0)
            tp = NormalizeDouble(entry - (sl - entry) * InpRiskRewardRatio, _Digits);
         trade_Send(TRADE_ACTION_PENDING, ORDER_TYPE_SELL_STOP,
                    entry, sl, tp, lot,
                    StringFormat("FP_Sell_HFT%d", (int)MathRound(-hftScore)));
        }
     }
  }
      ...
      if(MathAbs(newSL - curSL) > _Point / 2.0)
        {
         ...
         if(!OrderSend(req, res))
            Print("Footprint EA — ManagePositions modify failed: retcode=", res.retcode);
        }
     }
  }
