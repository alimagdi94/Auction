   ...
   for(int v = 0; v < visBars; v++)
     {
      ...
      string lbl = isBuySignal
                   ...
                   : "  SELL " + IntegerToString((int)MathRound(-hftScore));
      ...
      // ── Step 3: Signal ball on the left of the banner ────────────
      int aMX = x1 + InpSigBallMinRadius + 5;
      int aMY = midBarY;
      int miniR = MathMax(InpSigBallMinRadius, InpSigBallRadius / 2);
      DrawRings(aMX, aMY, miniR, sigColor, 220);

      // ── Step 4: Label with shadow ──────────────────────────────────
      canvas.FontSet("Consolas", 11, FW_BOLD);
      int tX = x1 + InpSigBallMinRadius * 2 + 12;
      canvas.TextOut(tX + 1, midBarY + 1, lbl, cShadow, TA_LEFT | TA_VCENTER);
      canvas.TextOut(tX,     midBarY,     lbl, cWhite,  TA_LEFT | TA_VCENTER);

      // ── Step 5: External ball marker outside the bar ─────────────
      int ballR = MathMax(InpSigBallMinRadius, InpSigBallRadius);
      if(isBuySignal)
        {
         int ballY = barBot + ballR + 4;
         DrawRings(wx, ballY, ballR, sigColor, 220);
        }
      else
        {
         int ballY = barTop - ballR - 4;
         DrawRings(wx, ballY, ballR, sigColor, 220);
        }
     }
  }
