...
void DrawBar(int bi, int shift, int barW)
  {
   ...
   if(g_signalsEnabled)
     {
      // ── Frequency gate: skip bars that are too close to the last signal ──
      // This prevents signal spam on every bar and enforces minimum spacing.
      ...
      int    currentScore = ComputeOFScore(bi);  // still used for OFS label

      bool isBuySignal  = freqGatePass && (hftScore >=  (double)g_signalThreshold);
      bool isSellSignal = freqGatePass && (hftScore <= -(double)g_signalThreshold);

      // Record the bar index the moment a signal fires so the gate works correctly
         ...
         if(bi == nBarsTotal - 1)
           {
            ...
            string msg  = StringFormat("Footprint HFT Signal [%s] %s | HFT:%d OFS:%d | Thr:%d | %s",
                                       _Symbol, dir, displayScore, currentScore, g_signalThreshold,
