   ...
   if(g_signalsEnabled)
     {
      ...
      bool isBuySignal  = (currentScore >= InpSignalThreshold);
      bool isSellSignal = (currentScore <= sellThresh);
      ...
      if(isBuySignal || isSellSignal)
        {
         ...
         if(bi == nBarsTotal - 1)
           {
            if(bi - g_lastSignalBar >= g_signalFreqBars)
              {
               g_lastSignalBar = bi;
               string dir = isBuySignal ? "BUY" : "SELL";
               string msg  = StringFormat("Footprint Signal [%s] %s | Score:%d | %s",
