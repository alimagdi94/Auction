//+------------------------------------------------------------------+
//|                                                   OrderFlow.mq5   |
//|   Order Flow Suite — Footprint + Bookmap Overlay                  |
//|   Volume / Delta / Bid x Ask + Concentric Ring Trade Bubbles     |
...
#property description "Order Flow Suite — Footprint + Bookmap Concentric Ring Overlay"
...
input int    InpHistoryBars     = 50;           // History bars to load
...
#define FP_MIN_CELL_H           20   // Min cell height (px) for readability
...
int   g_btnBubX1, g_btnBubY1, g_btnBubX2, g_btnBubY2;
int   g_mouseX = -1, g_mouseY = -1;
double g_vaPercent = 0;   // 0 = use InpVAPercent; else runtime VA%
bool   g_visible   = true;
bool   g_showBubbles = true;  // Bookmap bubble overlay toggle
bool   g_showFootprint = true; // Footprint cells/frames toggle
double g_globalMaxCellVol = 1.0;  // Global max cell volume across all bars (for ring normalization)
int    g_btnFpX1, g_btnFpY1, g_btnFpX2, g_btnFpY2;
...
int InsertBar(datetime bt)
  {
   ...
   for(int i = n; i > pos; i--)
     {
      g_bars[i] = g_bars[i - 1]; // Native MQL5 struct assignment
     }
...
void AccumulateTick(int bi, double price, long vol, bool isBuy, bool isSell)
  {
   ...
   // Track global max for Bookmap ring normalization
   double cv = (double)g_bars[bi].levels[idx].total_vol;
   if(cv > g_globalMaxCellVol) g_globalMaxCellVol = cv;
...
void DrawBar(int bi, int shift, int barW)
  ...
  if(g_showFootprint)
  {
      ...
      if(!skipText)
        {
         ...
         if(g_mode == FOOT_CHART_BIDASK)
           ...
           {
            color volBgCol = LerpColor(InpBidBaseColor, InpBidHighColor, levelIntensity);
            if(!inVA) volBgCol = InpOutOfVAColor;

            uint tCVol = IsColorDark(volBgCol) ? FpARGB(InpTextDarkBg, (250 * opaScale) / 255)
                                               : FpARGB(InpTextLightBg, (250 * opaScale) / 255);
            if(!pl.is_absorption && i != pocIdx && inVA)
               tCVol = colBaseText;
   ...
   if(g_showFootprint && vaLoIdx >= 0 && vaHiIdx >= 0)
     {
  ...
  } // end g_showFootprint cell block

   // ---- Bookmap: Concentric ring bubbles (separate resolution) ----
   if(g_showBubbles)
     {
      ...
      // Single-pass bucket aggregation: group footprint levels into
      // coarser bubble buckets (bubbleStep), then draw one ring per bucket.
      double curBucket   = -1e30;
      long   bAsk = 0, bBid = 0, bTot = 0;
      int    bYSum = 0, bYCnt = 0;

      for(int i = 0; i <= len; i++)  // <= to flush last bucket
        {
         double bucketId = (i < len) ? MathFloor(g_bars[bi].levels[i].price / bubStep) : -1e30;

         // Bucket boundary: flush accumulated bucket
         if(bucketId != curBucket && bTot > 0)
           {
            if(bTot >= (long)minThresh && bYCnt > 0)
              {
               int ycr = bYSum / bYCnt;
               double vr = MathPow((double)bTot / globalNorm, 0.55);
               if(vr > 1.0) vr = 1.0;
         ...
         // Accumulate current level into bucket
         if(i < len)
           {
            double p = g_bars[bi].levels[i].price;
            if(p >= barLo && p <= barHi)
              {
               curBucket = bucketId;
               bAsk += g_bars[bi].levels[i].ask_vol;
               bBid += g_bars[bi].levels[i].bid_vol;
               bTot += g_bars[bi].levels[i].total_vol;
               bYSum += (g_scratchY1[i] + g_scratchY2[i]) / 2;
               bYCnt++;
              }
           }
        }
...
void LayoutPanel(int cw, int ch)
  {
   ...
   int panelW = pad + (btnW * 13 + btnGap * 12) + pad;
   ...
   g_btnBubX1 = g_btnVAX2 + btnGap;
   g_btnBubY1 = g_btnZoomOutY1;
   g_btnBubX2 = g_btnBubX1 + btnW;
   g_btnBubY2 = g_btnZoomOutY2;

   g_btnFpX1 = g_btnBubX2 + btnGap;
   g_btnFpY1 = g_btnZoomOutY1;
   g_btnFpX2 = g_btnFpX1 + btnW;
   g_btnFpY2 = g_btnZoomOutY2;
  }
...
void DrawPanel()
  {
   ...
   bool hoveredBub      = HitTest(g_mouseX, g_mouseY, g_btnBubX1, g_btnBubY1, g_btnBubX2, g_btnBubY2);
   bool hoveredFp       = HitTest(g_mouseX, g_mouseY, g_btnFpX1, g_btnFpY1, g_btnFpX2, g_btnFpY2);
   ...
   uint fpBorder = g_showFootprint ? FpARGB(C'80,200,120', 230) : FpARGB(C'200,80,80', 230);
   if(hoveredFp)
     {
      fpFill   = hoverFill;
      fpBorder = hoverBorder;
     }
   canvas.FillRectangle(g_btnFpX1, g_btnFpY1, g_btnFpX2, g_btnFpY2, fpFill);
   canvas.Rectangle(g_btnFpX1, g_btnFpY1, g_btnFpX2, g_btnFpY2, fpBorder);
   canvas.TextOut(g_btnFpX1 + btnCenterX, btnCenterY,
                  "FP", FpARGB(clrWhite, 210), TA_CENTER | TA_VCENTER);
  }
...
int OnInit()
  {
   ...
   g_showFootprint= InpShowFootprint;
   ...
   if(!canvas.CreateBitmapLabel(g_name, 0, 0, w, h,
     ...
     {
      Alert("OrderFlow: Canvas creation failed. Chart may be too small.");
      return INIT_FAILED;
     }
   ...
   if(id == CHARTEVENT_KEYDOWN)
   ...
   else if(id == CHARTEVENT_CLICK)
     {
      ...
      // Footprint toggle
        ...
        {
         g_showFootprint = !g_showFootprint;
         g_dirty = true;
        }
