...
input bool   InpUseSoftStop         = true;    // EA-managed hidden stop (not placed with broker)
input double InpSoftStopPips        = 120.0;   // Floating loss in pips before EA closes the position
input bool   InpUseMaxHoldTime      = true;    // Close trades exceeding maximum holding duration
input int    InpMaxHoldMinutes      = 720;     // Maximum holding time in minutes (720 = 12 hours)
input double InpMaxFloatingLoss     = 300.0;   // Portfolio floating loss guard in account currency (0 = disabled)
...
bool     g_floatingLossHalted = false;    // set by portfolio equity guard — blocks new entries
...
void RiskStateSave()
  {
   ...
   GlobalVariableSet(GVKey("FloatHalted"),   g_floatingLossHalted ? 1.0 : 0.0);
...
void RiskStateLoad()
  {
   ...
   if(GlobalVariableCheck(GVKey("FloatHalted")))
      g_floatingLossHalted = (GlobalVariableGet(GVKey("FloatHalted")) != 0.0);
...
bool CheckRiskConditions(bool isBuy)
  {
   ...
   if(g_equityHalted || g_sessionHalted || g_dailyLossHalted || g_floatingLossHalted) return false;
      ...
      if(g_dayStartDay != curDay)
        {
         ...
         g_floatingLossHalted = false;   // reset portfolio guard on new day
...
//+------------------------------------------------------------------+
//| ClosePosition — EA-managed market close with retry               |
//|   Used by soft-stop, max-hold-time, and portfolio equity guard   |
//+------------------------------------------------------------------+
bool ClosePosition(ulong ticket)
  {
   ...
   req.position = ticket;
   req.magic    = g_Magic;
   req.comment  = "FP_RiskClose";
   req.type_filling = GetBrokerFillingMode();

   MqlTick lv;
   if(!SymbolInfoTick(_Symbol, lv)) return false;
   req.price = (req.type == ORDER_TYPE_SELL) ? lv.bid : lv.ask;
   ...
   if(!ok)
      LogTradeExec(StringFormat("ClosePosition FAILED: ticket=%I64u retcode=%u", ticket, res.retcode));
   return ok;
  }

//+------------------------------------------------------------------+
//| CloseAllPositions — close every position for this EA/symbol      |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| ManagePositions — break-even, trailing, soft stop, hold time     |
//+------------------------------------------------------------------+
void ManagePositions()
  {
   ...
   // If total floating loss exceeds InpMaxFloatingLoss, close ALL and halt.
   if(InpMaxFloatingLoss > 0.0 && !g_floatingLossHalted)
     {
      ...
      if(totalFloat < 0.0 && MathAbs(totalFloat) >= InpMaxFloatingLoss)
        {
         LogRisk(StringFormat(
            "EQUITY GUARD TRIGGERED | floating=%.2f limit=%.2f — closing all positions.",
            totalFloat, InpMaxFloatingLoss));
         CloseAllPositions();
         g_floatingLossHalted = true;
         RiskStateSave();
         return;   // all positions closed — nothing left to manage
        }
     }
