...
//|   EVOLVED: OrderFlowAlpha signal logic · High-probability signals  |
//|   · Push notifications (SendNotification) · No trading/EA code  |
//+------------------------------------------------------------------+
#property copyright "Ali Magdy"
#property version   "6.20"
#property description "Footprint Evolved v6.20 — Buy & sell score labels · Per-direction signals · Production ready"
...
input int    InpNakedPOCLookahead = 500;             // Max bars to look ahead for retest (0 = unlimited, reduces O(n²))
...
input group "HFT Signal Weights (%)"
input double InpHFTWtOFS      = 30.0;   // HFT weight: OFS (0–100)
input double InpHFTWtDeltaDiv = 20.0;   // HFT weight: Delta/Divergence
input double InpHFTWtPOC     = 15.0;   // HFT weight: POC position
input double InpHFTWtAbs     = 10.0;   // HFT weight: Absorption at extremes
input double InpHFTWtExh     = 10.0;   // HFT weight: Exhaustion
input double InpHFTWtCVD     = 15.0;   // HFT weight: CVD slope
input double InpHFTCVDScale  = 2.25;   // CVD slope scale into [-1,1]

input group "Push Notifications"
input bool   InpPushNotify        = true;            // Send MT5 push when valid signal appears
input bool   InpPushOnlyNewBar    = true;            // Notify only once per new bar (avoid spam)
...
// Conviction result for signal quality (OrderFlowAlpha logic)
...
// Signal & display tuning (industry-standard defaults)
#define FP_HFT_CVD_SLOPE_SCALE  2.25   // CVD slope scaled into [-1,1] for HFT component
#define FP_DELTA_GRAD_MAG_SCALE 2.5    // Delta ratio scale for gradient alpha
#define FP_EXTREME_LEVELS_MAX   3      // Max levels at bar high/low for absorption/exhaustion check
#define FP_SIGNAL_PLACEMENT_MAX 64    // Max overlay labels for collision avoidance
...
int   g_btnSigX1,  g_btnSigY1,  g_btnSigX2,  g_btnSigY2;  // Signal toggle button
int   g_btnFreqX1, g_btnFreqY1, g_btnFreqX2, g_btnFreqY2; // Signal frequency (min bars)
int   g_btnBuyTX1, g_btnBuyTY1, g_btnBuyTX2, g_btnBuyTY2; // Buy HFT threshold
int   g_btnSelTX1, g_btnSelTY1, g_btnSelTX2, g_btnSelTY2; // Sell HFT threshold
bool  g_userHideSignals = false;  // Runtime toggle: hide/show signal markers
...
// --- Signal & push state (OrderFlowAlpha-style) ---
bool     g_signalsEnabled     = true;
int      g_signalFreqBars     = 3;
int      g_signalThreshold    = 60;   // buy: HFT >= this
int      g_signalThresholdSell = 60; // sell: HFT <= -this (0 at init = use buy)
datetime g_lastSignalBarTime  = 0;
int      g_sigCacheBarIdx    = -1;
long     g_sigCacheVol       = -1;
datetime g_lastNotifiedBarTime = 0;  // push: only notify once per bar
bool     g_lastNotifiedBuy   = false;
...
void EvalSignalDirection(double hftScore, bool &isBuy, bool &isSell); // HFT → buy/sell flags
void EvalAndFireSignal();       // evaluate live bar, draw arrow, send push
...
//| Lookahead capped by InpNakedPOCLookahead to avoid O(n²) lag.   |
//+------------------------------------------------------------------+
void ComputeNakedPOCs()
  {
   int n = ArraySize(g_bars);
   int lookEnd = (InpNakedPOCLookahead > 0) ? InpNakedPOCLookahead : n;
   for(int i = 0; i < n; i++)
     {
      ...
      bool   retested = false;
      // Check bars that opened after this one, up to lookahead cap (avoids O(n²) on large history)
      int jEnd = (InpNakedPOCLookahead > 0)
                 ? MathMin(n, i + 1 + lookEnd)
                 : n;
      for(int j = i + 1; j < jEnd && !retested; j++)
        {
...
void ReloadHistory()
  {
   g_dirty = false;
   // Purge signal arrows so they are recreated from fresh data
...
//+------------------------------------------------------------------+
//| Helper: absorption at bar extremes (low/high). Call after        |
//| ComputeBarSignals(bi). Descending sort: 0=high, len-1=low.        |
//+------------------------------------------------------------------+
void GetAbsorptionAtExtremes(int bi, bool &atLow, bool &atHigh)
  {
   atLow  = false;
   atHigh = false;
   int len = g_bars[bi].level_count;
   ...
   for(int i = len - chk; i < len; i++)
      if(i >= 0 && g_bars[bi].levels[i].is_absorption) { atLow  = true; break; }
   for(int i = 0; i < chk; i++)
      if(i < len && g_bars[bi].levels[i].is_absorption) { atHigh = true; break; }
  }
...
//|  D. Absorption LOCATION  — at bar low = bullish, at bar high = bearish (industry standard)
//+------------------------------------------------------------------+
int ComputeOFScore(int bi)
  {
   ...
   // D: absorption by LOCATION (symmetric buy/sell). Descending sort: 0=high, len-1=low.
   bool absOfsLow = false, absOfsHigh = false;
   GetAbsorptionAtExtremes(bi, absOfsLow, absOfsHigh);
...
//| Symmetric buy/sell; levels[] descending (0=high, len-1=low).     |
//| Weights: OFS 30% | Delta/Div 20% | POC 15% | Abs 10% | Exh 10% | CVD 15%
//+------------------------------------------------------------------+
double ComputeHFTSignal(int bi, int preOFS = -1)
  {
   ...
   double c1 = ((double)ofs - 50.0) / 50.0;  // symmetric: 0→-1, 50→0, 100→+1
   ...
   // C3: POC position — low POC = bullish, high POC = bearish (symmetric)
   double c3 = 0.0;
   ...
   // Weights from inputs (%), normalized to sum 1.0
   ...
   w1 /= wSum; w2 /= wSum; w3 /= wSum; w4 /= wSum; w5 /= wSum; w6 /= wSum;
...
//+------------------------------------------------------------------+
//| Conviction label + component count (OrderFlowAlpha)              |
//| Descending sort: low price = high index (len-1), high = 0.      |
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| Append segment to conviction label with '+' separator             |
//+------------------------------------------------------------------+
void AppendConvictionLabel(string &label, const string part)
  {
   label += (label == "" ? "" : "+") + part;
  }
...
ConvictionResult GetConvictionResult(int bi, bool isBuy)
  {
   ...
   if(g_bars[bi].is_delta_divergence)
     { AppendConvictionLabel(out.label, "DeltaDiv"); out.componentCount++; }
   if(g_bars[bi].is_naked_poc)
     { AppendConvictionLabel(out.label, "NakedPOC"); out.componentCount++; }
   ...
   if(hasStackDir)
     { AppendConvictionLabel(out.label, isBuy ? "StackBuy" : "StackSell"); out.componentCount++; }

   bool absorbLo = false, absorbHi = false;
   GetAbsorptionAtExtremes(bi, absorbLo, absorbHi);
   if((isBuy && absorbLo) || (!isBuy && absorbHi))
     { AppendConvictionLabel(out.label, isBuy ? "AbsLow" : "AbsHigh"); out.componentCount++; }
...
//+------------------------------------------------------------------+
//| EvalSignalDirection — map HFT score to buy/sell signal flags     |
//+------------------------------------------------------------------+
void EvalSignalDirection(double hftScore, bool &isBuy, bool &isSell)
  {
   isBuy  = (hftScore >=  (double)g_signalThreshold);
   isSell = (hftScore <= -(double)g_signalThresholdSell);
  }

//+------------------------------------------------------------------+
//| EvalAndFireSignal — evaluate live bar, draw arrow, push notify  |
...
void EvalAndFireSignal()
  {
   ...
   EvalSignalDirection(hftScore, isBuySignal, isSellSignal);
   if(!isBuySignal && !isSellSignal) return;

   ConvictionResult conv = GetConvictionResult(bi, isBuySignal);
   if(conv.componentCount < InpMinConvictionComp) return;

   g_lastSignalBarTime = g_bars[bi].bar_time;

   string objName = StringFormat("FP_Sig_%s_%I64d", isBuySignal ? "B" : "S", (long)g_bars[bi].bar_time);
   if(ObjectFind(g_chart, objName) < 0)
     {
      // Push arrows well below/above the candle body to avoid covering price action
      double arrowPrice = isBuySignal ? g_bars[bi].low  - g_step * 5.5 : g_bars[bi].high + g_step * 5.5;
      // 233 = Wingdings ↑ (large up arrow, buy below bar pointing up)
      // 234 = Wingdings ↓ (large down arrow, sell above bar pointing down)
      int    arrowCode  = isBuySignal ? 233 : 234;
      color  arrowCol   = isBuySignal ? InpSignalBuyColor : InpSignalSellColor;
      if(ObjectCreate(g_chart, objName, OBJ_ARROW, 0, g_bars[bi].bar_time, arrowPrice))
        {
         ObjectSetInteger(g_chart, objName, OBJPROP_ARROWCODE,  arrowCode);
         ObjectSetInteger(g_chart, objName, OBJPROP_COLOR,      arrowCol);
         ObjectSetInteger(g_chart, objName, OBJPROP_WIDTH,      4);
         ObjectSetInteger(g_chart, objName, OBJPROP_BACK,       true);   // behind price candles
         ObjectSetInteger(g_chart, objName, OBJPROP_SELECTABLE, false);
         // Respect runtime signal hide toggle
         if(g_userHideSignals)
            ObjectSetInteger(g_chart, objName, OBJPROP_TIMEFRAMES, OBJ_NO_PERIODS);
         ObjectSetString( g_chart, objName, OBJPROP_TOOLTIP,
                          StringFormat("%s Signal  |  %s %s  |  HFT Score: %d  |  OFS: %d  |  Price: %s  |  Freq filter: %d bars",
                                       isBuySignal ? "▲ BUY" : "▼ SELL", _Symbol,
                                       EnumToString(Period()),
                                       (int)MathRound(isBuySignal ? hftScore : -hftScore), ofsScore,
                                       DoubleToString(iClose(_Symbol, PERIOD_CURRENT, 0), _Digits),
                                       g_signalFreqBars));
        }
     }

   if(InpPushNotify && (InpPushOnlyNewBar ? (g_lastNotifiedBarTime != g_bars[bi].bar_time) : true))
     {
      g_lastNotifiedBarTime = g_bars[bi].bar_time;
      g_lastNotifiedBuy     = isBuySignal;
      string tfStr = EnumToString(Period());
      StringReplace(tfStr, "PERIOD_", "");
      string msg = StringFormat("OrderFlow Signal Detected\nSymbol: %s\nTimeframe: %s\nSignal: %s",
                                _Symbol, tfStr, isBuySignal ? "BUY" : "SELL");
      if(!SendNotification(msg))
         Print("FootPrintEvolved: SendNotification failed ", GetLastError());
     }
  }

//+------------------------------------------------------------------+
//| DrawSignalMarkersPass — canvas labels for visible signal bars    |
//| Production: per-direction frequency so sell score shows like buy. |
//+------------------------------------------------------------------+
void DrawSignalMarkersPass(int visBars, int firstVis, int barW)
  {
   if(!g_signalsEnabled) return;
   if(g_userHideSignals)  return;   // runtime toggle from the Sig button
   int lastDrawnBarBuy  = -9999;
   int lastDrawnBarSell  = -9999;
   int placedCount = 0;
   int placedX[FP_SIGNAL_PLACEMENT_MAX], placedY[FP_SIGNAL_PLACEMENT_MAX], placedHalfW[FP_SIGNAL_PLACEMENT_MAX];
   int cw = canvas.Width();
   ...
   for(int v = 0; v < visBars; v++)
     {
      ...
      if(bi < 0) continue;
      if(g_bars[bi].level_count == 0 || g_bars[bi].total_vol == 0) continue;
      if(!g_bars[bi].sorted) ComputeBarSignals(bi);
      ...
      if(isBuySignal  && (bi - lastDrawnBarBuy  < g_signalFreqBars)) continue;
      if(isSellSignal && (bi - lastDrawnBarSell < g_signalFreqBars)) continue;
      if(isBuySignal)  lastDrawnBarBuy  = bi;
      if(isSellSignal) lastDrawnBarSell = bi;
      ...
      // Slightly lighter tint for the label background glow
      color  glowColor = isBuySignal ? C'0,60,30' : C'60,10,20';
      ...
      string dirStr    = isBuySignal ? "BUY" : "SELL";
      string pxStr     = DoubleToString(iClose(_Symbol, PERIOD_CURRENT, shift), _Digits);
      string hint      = StringFormat("%s %s HFT:%d OFS:%d @ %s", arrowStr, dirStr, hftDisp, ofsScore, pxStr);

      canvas.FontSet("Consolas", (barW >= 18) ? 13 : (barW >= 12 ? 11 : 10), FW_BOLD);
      int twHint = 0, thHint = 0;
      canvas.TextSize(hint, twHint, thHint);
      int boxW = (twHint > 0 ? twHint : (int)(barW * 2));
      int boxH = (thHint > 0 ? thHint : 12);
      const int gapY = 2, stepY = 12;
      int hx = barCX;
      // Place label OUTSIDE the bar range — below for buys, above for sells
      int hy = isBuySignal ? (barBot + gapY + 4) : (barTop - boxH - gapY - 4);
      int halfBoxW = boxW / 2;
      if(hx < 2 + halfBoxW) hx = 2 + halfBoxW;
      if(hx > cw - 2 - halfBoxW) hx = cw - 2 - halfBoxW;
      hy = MathMax(2, MathMin(ch - boxH - 2, hy));

      int tries = 0;
      while(tries < 10)
        {
         bool conflict = false;
         for(int i = 0; i < MathMin(placedCount, FP_SIGNAL_PLACEMENT_MAX); i++)
           {
            if(MathAbs(hx - placedX[i]) <= halfBoxW + placedHalfW[i] + 2 &&
               MathAbs(hy - placedY[i]) <= boxH + 1) { conflict = true; break; }
           }
         if(!conflict) break;
         hy += isBuySignal ? stepY : -stepY;
         hy = MathMax(2, MathMin(ch - boxH - 2, hy));
         if((isBuySignal && hy > ch - boxH - 2) || (!isBuySignal && hy < 2)) break;
         tries++;
        }
      if(placedCount < FP_SIGNAL_PLACEMENT_MAX)
        { placedX[placedCount]=hx; placedY[placedCount]=hy; placedHalfW[placedCount]=halfBoxW; placedCount++; }

      // --- Enhanced label: outer glow + filled pill + border + text ---
      int pad = 4;
      int rx1 = hx - halfBoxW - pad;
      int rx2 = hx + halfBoxW + pad;
      int ry1 = hy - 2;
      int ry2 = hy + boxH + 2;

      // Outer glow (slightly expanded, low alpha)
      canvas.FillRectangle(rx1 - 2, ry1 - 2, rx2 + 2, ry2 + 2, FpARGB(sigColor, 35));
      canvas.FillRectangle(rx1 - 1, ry1 - 1, rx2 + 1, ry2 + 1, FpARGB(sigColor, 50));
      // Filled dark background pill
      canvas.FillRectangle(rx1, ry1, rx2, ry2, FpARGB(glowColor, 220));
      // Crisp accent border in signal color
      canvas.Rectangle(rx1, ry1, rx2, ry2, FpARGB(sigColor, 220));

      // Shadow then text
      canvas.TextOut(hx + 1, hy + 1, hint, cShadow, TA_CENTER | TA_TOP);
      canvas.TextOut(hx,     hy,     hint, cText,   TA_CENTER | TA_TOP);

      // Small vertical stem connecting label to bar edge (connector line)
      int stemX  = hx;
      int stemY1 = isBuySignal ? barBot      : ry2 + 1;
      int stemY2 = isBuySignal ? ry1 - 1     : barTop;
      if(stemY2 > stemY1)
         canvas.LineVertical(stemX, stemY1, stemY2, FpARGB(sigColor, 90));
     }
  }
   ...
   if(InpDeltaGradient && g_bars[bi].total_vol > 0)
     {
      ...
      int    gAlpha = (int)(InpDeltaGradMaxAlpha * MathMin(1.0, dRat * FP_DELTA_GRAD_MAG_SCALE));
...
void LayoutPanel(int cw, int ch)
  {
   ...
   // 1 history OBJ_EDIT + 17 buttons = 18 items
   ...
   // Sig — signal show/hide toggle
   g_btnSigX1 = x; g_btnSigY1 = y1; g_btnSigX2 = x + btnW; g_btnSigY2 = y2;
   x += btnW + btnGap;

   // Group 7: Signal parameters ------------------------------------
   g_btnFreqX1 = x; g_btnFreqY1 = y1; g_btnFreqX2 = x + btnW; g_btnFreqY2 = y2;
   x += btnW + btnGap;

   g_btnBuyTX1 = x; g_btnBuyTY1 = y1; g_btnBuyTX2 = x + btnW; g_btnBuyTY2 = y2;
   x += btnW + btnGap;

   g_btnSelTX1 = x; g_btnSelTY1 = y1; g_btnSelTX2 = x + btnW; g_btnSelTY2 = y2;
...
void DrawPanel()
  {
   ...
   // After Sync | after Mode | after VA | after Lbl | after Lock | after Sig
   ...
   sepPositions[5] = g_btnSigX2      + FP_PANEL_BTN_GAP / 2;
   for(int s = 0; s < 6; s++)
   ...
   bool hoveredFreq     = HitTest(g_mouseX, g_mouseY, g_btnFreqX1,     g_btnFreqY1,     g_btnFreqX2,     g_btnFreqY2);
   bool hoveredBuyT     = HitTest(g_mouseX, g_mouseY, g_btnBuyTX1,     g_btnBuyTY1,     g_btnBuyTX2,     g_btnBuyTY2);
   bool hoveredSelT     = HitTest(g_mouseX, g_mouseY, g_btnSelTX1,     g_btnSelTY1,     g_btnSelTX2,     g_btnSelTY2);
   ...
   // Sig — signal markers toggle (green = on, dim red = off)
   bool sigEffective = g_signalsEnabled && !g_userHideSignals;
   uint sigFill   = sigEffective ? FpARGB(C'10,80,40', 230)   : FpARGB(C'70,20,20', 230);
   uint sigBorder = sigEffective ? FpARGB(C'40,210,110', 235)  : FpARGB(C'180,50,60', 200);
   ...
   canvas.TextOut(g_btnSigX1 + btnCenterX, btnCenterY,
                  sigEffective ? "Sig" : "Sig", FpARGB(clrWhite, 210), TA_CENTER | TA_VCENTER);

   // --- Group 7: Signal parameters ---

   // Freq — min bars between repeated signals (violet accent)
   canvas.FillRectangle(g_btnFreqX1, g_btnFreqY1, g_btnFreqX2, g_btnFreqY2,
                        hoveredFreq ? hoverFill : FpARGB(C'40,20,65', 230));
   canvas.Rectangle(g_btnFreqX1, g_btnFreqY1, g_btnFreqX2, g_btnFreqY2,
                    hoveredFreq ? hoverBorder : FpARGB(C'150,80,220', 220));
   canvas.TextOut(g_btnFreqX1 + btnCenterX, btnCenterY,
                  "F:" + IntegerToString(g_signalFreqBars),
                  FpARGB(C'200,160,255', 230), TA_CENTER | TA_VCENTER);

   // BuyT — buy HFT threshold (green accent)
   canvas.FillRectangle(g_btnBuyTX1, g_btnBuyTY1, g_btnBuyTX2, g_btnBuyTY2,
                        hoveredBuyT ? hoverFill : FpARGB(C'10,50,25', 230));
   canvas.Rectangle(g_btnBuyTX1, g_btnBuyTY1, g_btnBuyTX2, g_btnBuyTY2,
                    hoveredBuyT ? hoverBorder : FpARGB(C'40,190,100', 220));
   canvas.TextOut(g_btnBuyTX1 + btnCenterX, btnCenterY,
                  "B>" + IntegerToString(g_signalThreshold),
                  FpARGB(C'80,230,140', 230), TA_CENTER | TA_VCENTER);

   // SelT — sell HFT threshold (red accent)
   canvas.FillRectangle(g_btnSelTX1, g_btnSelTY1, g_btnSelTX2, g_btnSelTY2,
                        hoveredSelT ? hoverFill : FpARGB(C'55,10,18', 230));
   canvas.Rectangle(g_btnSelTX1, g_btnSelTY1, g_btnSelTX2, g_btnSelTY2,
                    hoveredSelT ? hoverBorder : FpARGB(C'210,50,70', 220));
   canvas.TextOut(g_btnSelTX1 + btnCenterX, btnCenterY,
                  "S>" + IntegerToString(g_signalThresholdSell),
                  FpARGB(C'240,100,120', 230), TA_CENTER | TA_VCENTER);
  }
...
int OnInit()
  {
   ...
   g_signalThresholdSell= (InpSignalThresholdSell > 0)
                          ? MathMax(1, MathMin(99, InpSignalThresholdSell))
                          : g_signalThreshold;
                  ...
                  const double &dparam, const string &sparam)
  {
   ...
   if(id == CHARTEVENT_CHART_CHANGE)
   ...
   else if(id == CHARTEVENT_CLICK)
     {
      ...
      // Signal toggle: show/hide signal markers and arrows
      else if(HitTest(mx, my, g_btnSigX1, g_btnSigY1, g_btnSigX2, g_btnSigY2))
        {
         g_userHideSignals = !g_userHideSignals;
         // Hide or restore OBJ_ARROW signal objects on the chart
         int total = ObjectsTotal(g_chart, 0, OBJ_ARROW);
         for(int i = total - 1; i >= 0; i--)
           {
            string nm = ObjectName(g_chart, i, 0, OBJ_ARROW);
            if(StringFind(nm, "FP_Sig_") == 0)
               ObjectSetInteger(g_chart, nm, OBJPROP_TIMEFRAMES,
                                g_userHideSignals ? OBJ_NO_PERIODS : OBJ_ALL_PERIODS);
           }
         g_dirty = true;
         Render();
        }
      // Freq — cycle min bars between repeated signals: 1→2→3→5→10→1
      else if(HitTest(mx, my, g_btnFreqX1, g_btnFreqY1, g_btnFreqX2, g_btnFreqY2))
        {
         if(g_signalFreqBars <= 1)       g_signalFreqBars = 2;
         else if(g_signalFreqBars <= 2)  g_signalFreqBars = 3;
         else if(g_signalFreqBars <= 3)  g_signalFreqBars = 5;
         else if(g_signalFreqBars <= 5)  g_signalFreqBars = 10;
         else if(g_signalFreqBars <= 10) g_signalFreqBars = 20;
         else                            g_signalFreqBars = 1;
         g_dirty = true;
         Render();
        }
      // BuyT — cycle buy HFT threshold: 30→40→45→50→60→70→30
      else if(HitTest(mx, my, g_btnBuyTX1, g_btnBuyTY1, g_btnBuyTX2, g_btnBuyTY2))
        {
         if(g_signalThreshold <= 30)      g_signalThreshold = 40;
         else if(g_signalThreshold <= 40) g_signalThreshold = 45;
         else if(g_signalThreshold <= 45) g_signalThreshold = 50;
         else if(g_signalThreshold <= 50) g_signalThreshold = 60;
         else if(g_signalThreshold <= 60) g_signalThreshold = 70;
         else                             g_signalThreshold = 30;
         // Force redraw of signal arrows from scratch with new threshold
         g_sigCacheBarIdx = -1;
         g_sigCacheVol    = -1;
         g_dirty = true;
         Render();
        }
      // SelT — cycle sell HFT threshold: 30→40→45→50→60→70→30
      else if(HitTest(mx, my, g_btnSelTX1, g_btnSelTY1, g_btnSelTX2, g_btnSelTY2))
        {
         if(g_signalThresholdSell <= 30)      g_signalThresholdSell = 40;
         else if(g_signalThresholdSell <= 40) g_signalThresholdSell = 45;
         else if(g_signalThresholdSell <= 45) g_signalThresholdSell = 50;
         else if(g_signalThresholdSell <= 50) g_signalThresholdSell = 60;
         else if(g_signalThresholdSell <= 60) g_signalThresholdSell = 70;
         else                                 g_signalThresholdSell = 30;
         g_sigCacheBarIdx = -1;
         g_sigCacheVol    = -1;
         g_dirty = true;
         Render();
