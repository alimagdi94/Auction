   ...
   if(ObjectFind(g_chart, objName) < 0)
     {
      ...
      int    arrowCode  = isBuySignal ? 241 : 242;  // 241=▲ up arrow, 242=▼ down arrow (Wingdings)
      ...
      if(ObjectCreate(g_chart, objName, OBJ_ARROW, 0, g_bars[bi].bar_time, arrowPrice))
        {
         ...
         ObjectSetInteger(g_chart, objName, OBJPROP_WIDTH,      3);
         ...
         ObjectSetString( g_chart, objName, OBJPROP_TOOLTIP,
                          StringFormat("%s | %s (%s) HFT: %d OFS: %d @ %s",
...
void LayoutPanel(int cw, int ch)
  {
   ...
   // 1 history OBJ_EDIT + 14 buttons = 15 items
   int panelW = pad + (btnW * 15 + btnGap * 14) + pad;
