...
int ComputeOFScore(int bi)
  {
   ...
   //    Absorption at bar LOW  (indices len-chk..len-1) = buying into weakness = bullish.
   //    Absorption at bar HIGH (indices 0..chk-1)       = selling into strength = bearish.
   int  chkA = MathMin(3, len/3 + 1);
   bool absOfsLow = false, absOfsHigh = false;
   for(int i = len - chkA; i < len; i++)
      if(i >= 0 && g_bars[bi].levels[i].is_absorption) absOfsLow  = true;
   for(int i = 0; i < chkA; i++)
      if(i < len && g_bars[bi].levels[i].is_absorption) absOfsHigh = true;
...
double ComputeHFTSignal(int bi, int preOFS = -1)
  {
   ...
   for(int i = len - chk; i < len; i++)
      if(i >= 0 && g_bars[bi].levels[i].is_absorption) absLow = true;
   for(int i = 0; i < chk; i++)
      if(i < len && g_bars[bi].levels[i].is_absorption) absHigh = true;
   double c4 = 0.0;
...
ConvictionResult GetConvictionResult(int bi, bool isBuy)
  {
   ...
   if(g_bars[bi].is_delta_divergence)
     { out.label += (out.label==""?"":"+") + "DeltaDiv"; out.componentCount++; }
   if(g_bars[bi].is_naked_poc)
     { out.label += (out.label==""?"":"+") + "NakedPOC"; out.componentCount++; }
   ...
   if(hasStackDir)
     { out.label += (out.label==""?"":"+") + (isBuy ? "StackBuy" : "StackSell"); out.componentCount++; }
   ...
   if((isBuy && absorbLo) || (!isBuy && absorbHi))
     { out.label += (out.label==""?"":"+") + (isBuy ? "AbsLow" : "AbsHigh"); out.componentCount++; }
...
void DrawSignalMarkersPass(int visBars, int firstVis, int barW)
  {
   ...
   int placedCount = 0;
   int placedX[64], placedY[64], placedHalfW[64];
   int cw = canvas.Width();
   ...
   for(int v = 0; v < visBars; v++)
     {
      ...
      while(tries < 10)
        {
         bool conflict = false;
         for(int i = 0; i < MathMin(placedCount, 64); i++)
           {
