...
//|   v6.00 — OrderFlow Alpha signal engine integrated:              |
//|   • HFT Multi-Factor Signal (6 components, C1-C6)               |
//|   • CVD Momentum Slope · POC Gravity · Absorption extremes       |
//|   • Bid/Ask Exhaustion component · Delta Divergence factor       |
//|   • Adaptive ATR-scaled threshold · Conviction diversity gate    |
//|   • Bookmap-style ring markers · Signal hint cards               |
//|   • Runtime Sig toggle, FreqEdit, ThreshEdit in panel            |
//+------------------------------------------------------------------+
#property copyright "Ali Magdy"
#property version   "6.00"
#property description "Footprint Chart v6 — HFT Signal Engine · Adaptive Threshold · Conviction Gate · Ring Markers"
...
double ComputeHFTSignal(int bi, int preOFS = -1)
  {
   ...
   // Ascending sort: i < chk = LOW end (bullish); i >= len-chk = HIGH end (bearish)
   double c4 = 0.0;
   {
      ...
      if(ObjectCreate(g_chart, objName, OBJ_ARROW, 0,
                      g_bars[bi].bar_time, arrowPrice))
        {
         ...
         ObjectSetString( g_chart, objName, OBJPROP_TOOLTIP,
                          StringFormat("%s SIGNAL | %s\nHFT: %d | OFS: %d | %s\nPrice: %s | %s",
                                       isBuySignal ? "BUY" : "SELL",
                                       _Symbol,
                                       displayScore, ofsScore, conv.label,
                                       DoubleToString(SymbolInfoDouble(_Symbol, SYMBOL_BID), _Digits),
                                       TimeToString(g_bars[bi].bar_time, TIME_DATE|TIME_MINUTES)));
        }
     }
   ...
   for(int v = 0; v < visBars; v++)
     {
      ...
      // Compact hint text: "▲ HFT:23 OFS:42 @ 1.2345"
