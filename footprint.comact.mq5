...
input color  InpImbSellColor   = C'148,0,211';    // Sell Imbalance Glow (Bright Purple)
input color  InpImbBuyColor    = C'0,128,128';    // Buy Imbalance Glow (Bright Teal)
input color  InpStackedSellColor= C'148,0,211';   // Stacked Sell Zone Color
input color  InpStackedBuyColor = C'0,128,128';   // Stacked Buy Zone Color
...
input color  InpBullishFrame   = C'0,128,128';    // Bullish Session Frame (Teal)
input color  InpBearishFrame   = C'148,0,211';    // Bearish Session Frame (Purple)
...
void ReloadHistory()
{
   ...
   if (bars_total <= 0) return; // Prevent crash if history not loaded
                  ...
                  bool skipAlreadySeen, bool updateLastTimeMs)
{
   ...
   if(endIdx > ArraySize(ticks)) endIdx = ArraySize(ticks); // CRITICAL: Bounds check
...
void DrawBar(int bi, int shift, int barW)
{
   ...
   for(int i = 0; i < len; i++)
   {
      ...
      if(plMax.total_vol > maxLevelVol) maxLevelVol = plMax.total_vol;
      if(plMax.bid_vol   > maxBidVol)   maxBidVol   = plMax.bid_vol;
      if(plMax.ask_vol   > maxAskVol)   maxAskVol   = plMax.ask_vol;
   }

   datetime bt = iTime(_Symbol, PERIOD_CURRENT, shift);

   // Candle Wicks will be drawn at the end to establish visual priority
   int wx, wy_h, wy_l;
   ...
   for(int i = 0; i < len; i++)
   {
      ...
      double levelIntensity = (double)pl.total_vol / (double)maxLevelVol;
      double bidIntensity   = (maxBidVol > 0) ? (double)pl.bid_vol / (double)maxBidVol : 0.0;
      double askIntensity   = (maxAskVol > 0) ? (double)pl.ask_vol / (double)maxAskVol : 0.0;

      // Side-specific heatmap backgrounds: left = bid, right = ask
      ...
      // Grid separation (faded to reduce noise)
      canvas.LineHorizontal(x1, x2, y_bot, FpARGB(InpGridColor, 35));
      canvas.LineVertical(midX, y_top, y_bot, FpARGB(InpGridColor, 35));

      // Imbalance Subtle Glow / Border (Instead of blowing out the whole background)
      if(pl.is_imb_sell)
      {
         canvas.Rectangle(x1, y_top, midX, y_bot, FpARGB(InpImbSellColor, 200));
      }
      if(pl.is_imb_buy)
      {
         canvas.Rectangle(midX+1, y_top, x2, y_bot, FpARGB(InpImbBuyColor, 200));
      }
      ...
      // Unfinished Auctions (Extreme High / Low Markers) - Faded to reduce noise
      ...
      if(!skipText)
      {
         ...
         // Text Priority: Bright white only for significance. Muted otherwise.
         uint tCBid = (pl.is_imb_sell || pl.is_absorption || i == pocIdx) ? colWhiteText : colMutedText;
         uint tCAsk = (pl.is_imb_buy  || pl.is_absorption || i == pocIdx) ? colWhiteText : colMutedText;
         
         // Force dark text if overall bar intensity is extremely bright to keep contrast.
         if(levelIntensity > 0.85 && inVA) { tCBid = colBlackText; tCAsk = colBlackText; }
   ...
   // --- Candle Wicks (Pushed to end to ensure visual priority, sitting on top of heatmap) ---
                ...
                const int      &spread[])
{
   ...
   {
      ReloadHistory();
      if(!g_dirty) return 0; // History load failed, try again next tick
   }
   ...
   if(from_msc == 0)
   {
      // If we completely reset history, from_msc should be set by ReloadHistory.
      // If OnCalculate still sees from_msc == 0, fetch a safe recent window to avoid massive ArrayOutOfRange.
      long lookback = 60000;
      if(now_msc > lookback)
                  ...
                  const double &dparam, const string &sparam)
{
   ...
   else if(id == CHARTEVENT_CHART_CHANGE)
   {
      ...
      if(ArraySize(g_bars) > 0)
      {
         ...
         datetime expectedFirstBar = iTime(_Symbol, PERIOD_CURRENT, MathMin(InpHistoryBars, iBars(_Symbol, PERIOD_CURRENT) - 1));
         
         // Simple heuristic: if our earliest bar doesn't align with the expected timeframe boundary, we switched TFs.
